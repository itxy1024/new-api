...
export function createDefaultFavoriteCollection(
  now = Date.now()
): FavoriteCollection {
  return {
...
export function normalizeFavoriteCollections(
  value: unknown,
  now = Date.now()
): FavoriteCollection[] {
  ...
  for (const item of collections) {
    ...
    if (collection.id === ALL_FAVORITES_COLLECTION_ID || ids.has(collection.id))
      continue
    ...
    normalized.push({
      ...
      createdAt:
        typeof collection.createdAt === 'number' ? collection.createdAt : now,
      updatedAt:
        typeof collection.updatedAt === 'number' ? collection.updatedAt : now,
    })
  }
  return normalized
}

export function ensureDefaultFavoriteCollection(
...
function ensureDefaultNamedFavoriteCollection(
  collections: FavoriteCollection[],
  now = Date.now()
) {
...
export function normalizeFavoriteCollectionIds(value: unknown) {
  ...
  return Array.from(
    new Set(
      value.map(String).filter((id) => id && id !== ALL_FAVORITES_COLLECTION_ID)
    )
  )
}
...
export function getTaskFavoriteCollectionIds(
  task: TaskRecord,
...
export function normalizeLoadedFavoriteState(
  tasks: TaskRecord[],
  collections: unknown,
  preferredDefaultFavoriteCollectionId: unknown,
  now = Date.now()
) {
  let changed = false
  const initialCollections = ensureDefaultFavoriteCollection(
    normalizeFavoriteCollections(collections, now),
    now
  )
  const initialCollectionIds = new Set(
    initialCollections.map((collection) => collection.id)
  )
  const needsLegacyFallback = tasks.some(
    (task) =>
      Boolean(task.isFavorite) &&
      !normalizeFavoriteCollectionIds(task.favoriteCollectionIds).some((id) =>
        initialCollectionIds.has(id)
      )
  )
  ...
  const collectionIds = new Set(
    normalizedCollections.map((collection) => collection.id)
  )
  const fallbackId = needsLegacyFallback
    ? getDefaultNamedFavoriteCollectionId(normalizedCollections)
    : null
  const normalizedTasks = tasks.map((task) => {
    ...
    const ids =
      normalizedIds.length > 0
        ? normalizedIds
        : task.isFavorite && fallbackId
          ? [fallbackId]
          : []
    const isFavorite = ids.length > 0 || Boolean(task.isFavorite)
    const currentIds = Array.isArray(task.favoriteCollectionIds)
      ? task.favoriteCollectionIds
      : []
    if (
...
export function mergeFavoriteCollections(
  ...
  if (!importedCollections.length)
    return { collections: current, importedCollections }
  const collections = ensureDefaultFavoriteCollection(
    normalizeFavoriteCollections([...current, ...importedCollections], now),
    now
  )
...
export function deleteFavoriteCollectionState(
  input: DeleteFavoriteCollectionInput
) {
  if (!input.collectionId || input.collectionId === ALL_FAVORITES_COLLECTION_ID)
    return null
  if (
    input.collections.length <= 1 ||
    !input.collections.some(
  ...
  for (const task of input.tasks) {
    const currentIds = getTaskFavoriteCollectionIds(
      task,
      input.defaultFavoriteCollectionId
    )
    if (!currentIds.includes(input.collectionId)) continue
    const favoriteCollectionIds = currentIds.filter(
      (id) => id !== input.collectionId && collectionIds.has(id)
    )
    ...
    const updated = {
      ...
      isFavorite: favoriteCollectionIds.length > 0,
    }
  ...
  return {
    collections,
    defaultFavoriteCollectionId:
      input.defaultFavoriteCollectionId === input.collectionId
        ? (collections[0]?.id ?? null)
        : resolveDefaultFavoriteCollectionId(
            collections,
            input.defaultFavoriteCollectionId
          ),
    activeFavoriteCollectionId: activeDeleted
      ? null
      : input.activeFavoriteCollectionId,
    selectedFavoriteCollectionIds: activeDeleted
      ? []
      : input.selectedFavoriteCollectionIds.filter(

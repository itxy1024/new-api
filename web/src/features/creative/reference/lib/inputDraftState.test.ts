...
describe('input draft normalization', () => {
  ...
  it('filters drafts by known conversations while retaining keyed legacy drafts when conversations are external', () => {
    ...
    expect(
      Object.keys(
        normalizeAgentInputDrafts(value, [{ id: 'known' }, { id: 'empty' }])
      )
    ).toEqual(['known'])
    expect(Object.keys(normalizeAgentInputDraftsByKey(value))).toEqual([
      'known',
      'missing',
    ])
  })
...
describe('input draft mode and conversation transforms', () => {
  it('keeps gallery and agent drafts isolated across mode saves and restores', () => {
    ...
    expect(galleryDraft).toMatchObject({
      prompt: '画廊草稿',
      inputImages: [imageA],
      updatedAt: 100,
    })
    ...
    expect(restoreGalleryInputDraftState(galleryDraft)).toMatchObject({
      prompt: '画廊草稿',
      inputImages: [imageA],
    })
    expect(
      restoreAgentInputDraftState(agentDrafts, 'conversation-b')
    ).toMatchObject({ prompt: 'Agent 草稿', inputImages: [imageB] })
  ...
  it('syncs only the draft belonging to the active mode', () => {
    ...
    const gallery = syncActiveInputDraft(
      { ...base, appMode: 'gallery' },
      { prompt: '画廊' }
    )
    const agent = syncActiveInputDraft(
      { ...base, appMode: 'agent' },
      { prompt: 'Agent' }
    )

    expect(gallery.galleryInputDraft).toMatchObject({
      prompt: '画廊',
      updatedAt: 100,
    })
    expect(gallery).not.toHaveProperty('agentInputDrafts')
    expect(agent.agentInputDrafts?.['conversation-a']).toMatchObject({
      prompt: 'Agent',
      updatedAt: 100,
    })
...
describe('input draft image and mention transforms', () => {
  ...
  it('preserves mention position when an image id is replaced by an equivalent image', () => {
    ...
    expect(
      updateInputDraftImages(draft, [imageB], {
        equivalentImageIds: { [imageA.id]: imageB.id },
      })
    ).toMatchObject({
      prompt: `修改 ${getSelectedImageMentionLabel(0)}`,
      inputImages: [imageB],
...
describe('input draft persistence transforms', () => {
  it('filters stale drafts but always retains the active conversation', () => {
    ...
    expect(
      cleanStaleAgentInputDrafts(
        { active: activeDraft, stale: staleDraft, recent: recentDraft },
        'active',
        now
      )
    ).toEqual({

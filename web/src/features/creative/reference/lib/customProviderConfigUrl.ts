...
import { EMBEDDED_CONFIG_PREFIX } from './importableConfigUrl'
...
export function getCustomProviderConfigUrl(
...
export function loadEmbeddedDefaultConfig(
  ...
  const bytes = Uint8Array.from(
    atob(value.slice(EMBEDDED_CONFIG_PREFIX.length)),
    (char) => char.charCodeAt(0)
  )
  return importCustomProviderSettingsFromJson(
...
export async function loadCustomProviderSettingsFromUrl(
  ...
  if (settingsJsonText)
    return importCustomProviderSettingsFromJson(settingsJsonText, [], {
      deploymentConfig: true,
    })
  return null
}

...
describe('agent input builder', () => {
  ...
  it('uses only the current branch path', async () => {
    ...
    const input = await buildAgentApiInput({
      conversation: conversation(
        [first, branchA, branchB, currentRound],
        messages
      ),
      currentRound,
      tasks: [],
  ...
  it('replaces image mentions and appends matching image and XML references', async () => {
    ...
    const input = await buildAgentApiInput({
      conversation: conversation(
        [currentRound],
        [message(currentRound, `参考 ${getSelectedImageMentionLabel(1)} 生成`)]
      ),
      currentRound,
      tasks: [],
      loadImage,
    ...
    expect(serialized).toContain(
      '参考 <ref id=\\"round-3-reference-2\\" /> 生成'
    )
  ...
  it('restores historical output from a legacy task payload and stored generated image', async () => {
    ...
    const input = await buildAgentApiInput({
      ...
      tasks: [legacyTask],
      loadImage: async (id) =>
        id === 'generated-image' ? 'data:restored-image' : undefined,
    })
    const serialized = JSON.stringify(input)
  ...
  it('builds input safely from malformed legacy response output', async () => {
    ...
    const input = await buildAgentApiInput({
      conversation: conversation(
        [previous, currentRound],
        [message(previous, '历史请求'), message(currentRound, '继续')]
      ),
      currentRound,
      tasks: [legacyTask],
      loadImage: noImage,
  ...
  it('marks deleted historical output slots without restoring deleted payloads', async () => {
    const previous = round('round-1', 1, {
      ...
      responseOutput: [
        {
          type: 'message',
          content: [{ type: 'output_text', text: '完成两张图' }],
        },
        {
          type: 'image_generation_call',
          id: 'deleted-call',
          result: 'deleted-payload',
        },
        {
          type: 'image_generation_call',
          id: 'live-call',
    ...
    const input = await buildAgentApiInput({
      conversation: conversation(
        [previous, currentRound],
        [message(previous, '生成'), message(currentRound, '继续')]
      ),
      currentRound,
      tasks: [task('live-task', { outputImages: ['live-image'] })],
      loadImage: async (id) =>
        id === 'live-image' ? 'data:live-image' : undefined,
    })
    const serialized = JSON.stringify(input)
  ...
  it('treats a missing legacy task prompt as an empty reference prompt', async () => {
    ...
    expect(input).toEqual([
      ...
      {
        role: 'assistant',
        content: [{ type: 'output_text', text: '[No text response]' }],
      },
      {
        role: 'user',
        content: [
  ...
  it('builds the exact active-path continuation with historical and batch images', async () => {
    const first = round('round-1', 1, {
      ...
      responseOutput: [
        {
          type: 'message',
          content: [{ type: 'output_text', text: '基础完成' }],
        },
      ],
    })
    ...
    const tasks = [
      ...
      task('batch-done', {
        prompt: '批量图',
        outputImages: ['batch-image'],
        agentBatchCallId: 'batch-call',
      }),
      task('batch-running', {
        status: 'running',
        outputImages: ['partial-image'],
        agentBatchCallId: 'batch-call',
      }),
    ]
    const baseInput = await buildAgentApiInput({
      conversation: conversation(
        [first, sibling, currentRound],
        [
          message(first, '生成基础'),
          message(first, '消息中的回退回复', {
            id: 'assistant-1',
            role: 'assistant',
          }),
    ...
    expect(input).toEqual([
      ...
      },
      {
        ...
        content: [
          ...
          {
            type: 'input_text',
            text: '<ref id="round-1-image-1" prompt="基础图" />',
          },
        ],
      },
      {
        role: 'user',
        content: [{ type: 'input_text', text: '分支 B 批量继续' }],
      },
      functionCall,
      functionOutput,
      {
        ...
        content: [
          ...
          {
            type: 'input_text',
            text: '<ref id="round-2-image-2" prompt="批量图" />',
          },
        ],
      },
      {
  ...
  it('builds continuation with sanitized output and function results before the system message', async () => {
    ...
    expect(JSON.stringify(input[input.length - 1])).toContain(
      'Tool-call budget: 2/2 used.'
    )
  })
})

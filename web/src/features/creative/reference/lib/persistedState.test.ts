...
describe('persisted state codec', () => {
  it('rejects non-record unknown data and falls back field-by-field for an invalid record', () => {
    ...
    expect(result.state.agentConversations.map((item) => item.id)).toEqual([
  ...
  it('normalizes legacy conversations, active ID, and top-level Agent draft fallback', () => {
    ...
    expect(result.state.agentConversations[0].rounds[0]).toMatchObject({
      status: 'error',
      error: '上次请求已中断',
    })
    expect(result.state.agentInputDrafts['conversation-a']).toMatchObject({
      prompt: '旧版 Agent 草稿',
      inputImages: [imageA],
    })
  ...
  it('persists gallery and Agent drafts only when input persistence is enabled', () => {
      ...
      rounds: [
        {
          ...
          responseOutput: [
            {
              type: 'image_generation_call',
              result: 'legacy-conversation-base64',
            },
          ],
          status: 'done',
          error: null,
    ...
    const withLegacyConversation = createPersistedState(
      { ...source(), agentConversations: [legacyConversation] },
      true
    )
    ...
    ).not.toContain('legacy-conversation-base64')
  })
  ...
  it('filters malformed external response output before later payload stripping', () => {
    ...
    expect(JSON.stringify(encoded.agentConversations)).not.toContain(
      'external-base64'
    )
    expect(
      encoded.agentConversations?.[0].rounds[0].responseOutput
    ).toContainEqual({
      type: 'function_call_output',
      call_id: 'content-output',
      output: [
  ...
  it('restores old gallery and keyed Agent drafts while normalizing favorites and default ID', () => {
    ...
    const agent = normalizePersistedState(
      {
        ...
        agentInputDrafts: {
          'indexed-conversation': {
            prompt: 'IndexedDB 对话草稿',
            inputImages: [imageA],
            updatedAt: 1,
          },
          orphan: {
            prompt: '兼容保留的 keyed 草稿',
            inputImages: [],
            updatedAt: 1,
          },
        },
    ...
    expect(Object.keys(agent.state.agentInputDrafts)).toEqual([
      'indexed-conversation',
      'orphan',
    ])
    expect(emptyFavorites.state.favoriteCollections[0].id).toBe(
      DEFAULT_FAVORITE_COLLECTION_ID
    )
    expect(emptyFavorites.state.defaultFavoriteCollectionId).toBe(
      DEFAULT_FAVORITE_COLLECTION_ID
    )
  })

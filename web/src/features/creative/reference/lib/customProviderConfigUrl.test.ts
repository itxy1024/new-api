...
describe('custom provider config URL', () => {
  it('does not expose a JSON config URL to the browser loader', () => {
    expect(
      getCustomProviderConfigUrl('https://example.com/custom-provider.json')
    ).toBe('')
  })

  it('returns empty when default API URL is a normal API endpoint', () => {
    expect(getCustomProviderConfigUrl('https://api.example.com/v1')).toBe('')
  })

  it('detects importable URL values', () => {
    expect(isImportableConfigUrl('https://example.com/provider.json')).toBe(
      true
    )
  ...
  it('loads an embedded default config without fetching a URL', () => {
    const payload = JSON.stringify({
      customProviders: [
        { id: 'embedded', name: '内嵌服务商', submit: { path: 'generate' } },
      ],
      profiles: [],
    })
    ...
    expect(
      loadEmbeddedDefaultConfig(`embedded-config:${base64}`)?.customProviders[0]
    ).toMatchObject({
      id: 'embedded',
      name: '内嵌服务商',
    })
  ...
  it('accepts an empty embedded deployment snapshot', () => {
    const bytes = new TextEncoder().encode(
      JSON.stringify({ customProviders: [], profiles: [] })
    )
    const base64 = btoa(
      Array.from(bytes, (byte) => String.fromCharCode(byte)).join('')
    )

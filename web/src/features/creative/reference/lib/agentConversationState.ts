  ...
  const status =
    round.status === 'running'
      ...
      : round.status === 'error' || round.status === 'done'
        ? round.status
        : 'done'
  const responseOutput = Array.isArray(round.responseOutput)
    ? normalizeResponsesOutputItems(round.responseOutput)
    : undefined
...
export function normalizeAgentConversations(
  value: unknown
): AgentConversation[] {
    ...
    .map((conversation) => {
      ...
      const normalizedRounds = Array.isArray(conversation.rounds)
        ? conversation.rounds
            ...
            .filter((round): round is AgentRound => {
...
export function getAgentConversationTaskIds(

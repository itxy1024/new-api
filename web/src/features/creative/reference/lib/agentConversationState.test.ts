...
describe('agent conversation state', () => {
  it('stops traversing cyclic parent relationships', () => {
    const value = conversation(
      [round('round-a', 'round-b', 1), round('round-b', 'round-a', 2)],
      'round-a'
    )

    expect(getAgentRoundPath(value, 'round-a').map((item) => item.id)).toEqual([
      'round-b',
      'round-a',
    ])
  ...
  it.each([
    ...
    'normalizes $name compatibly',
    ({
      activeRoundId,
      parentRoundIds,
      expectedActiveRoundId,
      expectedParentRoundIds,
    }) => {
      ...
      expect(normalized[0].rounds.map((item) => item.parentRoundId)).toEqual(
        expectedParentRoundIds
      )
    }
  )
  ...
  it('filters malformed and orphaned records while preserving legacy field defaults', () => {
    ...
    expect(normalized[0].messages.map((message) => message.id)).toEqual([
      'user-round-a',
    ])
  })
  ...
  it('keeps the first entity when persisted IDs are duplicated', () => {
    ...
    expect(normalized[0].messages.map((message) => message.content)).toEqual([
      '第一条',
    ])
  })
  ...
  it('builds case-insensitive search text from conversation content', () => {
    ...
    expect(getConversationSearchText(value)).toBe(
      'project alpha\nmessage body\nround prompt'
    )
  })
})

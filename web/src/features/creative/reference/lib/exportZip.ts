...
type ZipFiles = Record<
  string,
  Uint8Array | [Uint8Array, { mtime: Date; level?: 0 }]
>
...
export async function buildExportZip(params: BuildExportZipParams) {
  ...
  const imageTasks = params.options.exportTasks
    ? (params.imageTasks ?? params.tasks)
    : []
  ...
  if (params.options.exportTasks) {
    for (const img of params.images) {
      ...
      const path = getUniqueImagePath(
        imageFileNameBases.get(img.id) || `image-${img.id}`,
        ext,
        usedImagePaths
      )
      const pathBase = path.slice('images/'.length, -(ext.length + 1))
      const createdAt =
        img.createdAt ?? imageCreatedAtFallback.get(img.id) ?? params.exportedAt
      imageFiles[img.id] = {
        path,
        createdAt,
        source: img.source,
      ...
      if (thumbnail?.thumbnailDataUrl) {
        ...
        imageFiles[img.id].height =
          imageFiles[img.id].height ?? thumbnail.height
        ...
        zipFiles[thumbnailPath] = [
          thumbnailBytes,
          { mtime: new Date(createdAt), level: 0 },
        ]
      }
    }
  }
  ...
  if (params.options.exportConfig && params.includeManifestData !== false)
    manifest.settings = params.settings
  if (params.options.exportTasks) {
    if (params.includeManifestData !== false || params.tasks.length)
      manifest.tasks = params.tasks
    if (
      params.includeManifestData !== false ||
      params.agentConversations.length
    )
...
export function getExportZipPlan(
  ...
  const safetyBytes = Math.min(
    EXPORT_PART_SAFETY_BYTES,
    Math.floor(maxBytes * 0.1)
  )
  const partBytes = Math.min(
    options.partBytes ?? DEFAULT_EXPORT_PART_BYTES,
    maxBytes - safetyBytes
  )
  const manifestBytes = getBaseManifestEstimatedBytes(params)
  const plannedTasks = params.options.exportTasks ? params.tasks : []
  const plannedConversations = params.options.exportTasks
    ? params.agentConversations
    : []
  ...
  const parts: ExportZipPlanPart[] = [
    { imageIds: [], tasks: [], agentConversations: [], includeBaseData: true },
  ]
  const sizes = [manifestBytes]
  const addItem = (
    bytes: number,
    errorMessage: string,
    append: (part: ExportZipPlanPart) => void
  ) => {
    let index = parts.length - 1
    const hasItems =
      parts[index].imageIds.length > 0 ||
  ...
  for (let index = 0; index < plannedTasks.length; index++) {
    addItem(
      taskBytes[index],
      '单条任务或 Agent 对话超过 2 GB，无法生成备份。',
      (part) => part.tasks.push(plannedTasks[index])
    )
  }
  for (let index = 0; index < plannedConversations.length; index++) {
    addItem(
      conversationBytes[index],
      '单条任务或 Agent 对话超过 2 GB，无法生成备份。',
      (part) => part.agentConversations.push(plannedConversations[index])
    )
  }
  for (const image of plannedImages) {
    addItem(
      image.bytes,
      `图片 ${image.id} 过大，无法放入小于 2 GB 的备份分片。`,
      (part) => part.imageIds.push(image.id)
    )
  }
  return parts
}
...
export async function readExportZipManifest(
  bytes: Uint8Array,
  validateFiles = true
): Promise<ExportData> {
  ...
  if (validateFiles)
    assertExportZipFiles(manifest, (path) => entryNames.has(path))
  return manifest
}

export async function readExportZip(
  bytes: Uint8Array
): Promise<ExportZipContents> {
  const files = await unzipFiles(bytes)
  const manifestBytes = files['manifest.json']
  if (!manifestBytes) throw new Error('ZIP 中缺少 manifest.json')
  const manifest = JSON.parse(strFromU8(manifestBytes)) as ExportData
...
function getBaseManifestEstimatedBytes(
  params: Omit<BuildExportZipParams, 'images' | 'thumbnailsByImageId'>
) {
  const manifest = {
    version: 3,
...
export function getExportImageEstimatedBytes(
  image: StoredImage,
  ...
  return (
    ...
    (thumbnail?.thumbnailDataUrl
      ? getDataUrlDecodedByteSize(thumbnail.thumbnailDataUrl)
      : 0) +
    ZIP_ENTRY_OVERHEAD_BYTES * (thumbnail?.thumbnailDataUrl ? 2 : 1)
  )
}

export function readExportZipFileAsDataUrl(
  files: Record<string, Uint8Array>,
  path: string
): string | null {
...
function getImageFileNameBases(tasks: TaskRecord[]) {
  ...
  for (const task of tasks)
    addImageFileNameBases(bases, task.outputImages || [], `task-${task.id}`)
  for (const task of tasks)
  ...
  for (const task of tasks) {
    if (task.maskImageId && !bases.has(task.maskImageId))
      bases.set(task.maskImageId, `task-${task.id}-mask`)
  }

  return bases
}

function addImageFileNameBases(
  ...
  for (let index = 0; index < ids.length; index++) {
    ...
    bases.set(
      ids[index],
      getNumberedFileNameBase(fileNameBase, index, ids.length)
    )
  }
}

function getUniqueImagePath(
  fileNameBase: string,
  ext: string,

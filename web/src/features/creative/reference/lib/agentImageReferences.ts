...
export function collectAgentRoundOutputImageSlots(
  round: AgentRound,
  tasks: TaskRecord[]
) {
...
export function extractAgentReferenceIds(text: string) {
  return Array.from(
    text.matchAll(AGENT_REF_TAG_RE),
    (match) => match[2]
  ).filter((id): id is string => Boolean(id))
}

export function resolveAgentPromptImageReferences(
  prompt: string,
  rounds: AgentRound[],
  tasks: TaskRecord[]
) {
...
export function replaceAgentPromptImageReferencesForApi(
  ...
  const replaceGeneratedReference = (
    ...
    const imageId = collectAgentRoundOutputImageSlots(sourceRound, tasks)[
      imageIndex
    ]
    if (!imageId)
      return getAgentRemovedReferenceTag(

...
function mapFalEndpoint(model: string, isEdit: boolean): string {
  const normalized =
    model.trim().replace(/^\/+/, '').replace(/\/+$/, '') || 'openai/gpt-image-2'
  return isEdit && !normalized.endsWith('/edit')
    ? `${normalized}/edit`
    : normalized
}

async function mapFalImageSize(
  size: string
...
async function createFalRequestInput(
  opts: CallApiOptions
  ...
  const input: Record<string, unknown> = {
    prompt: opts.prompt,
    image_size:
      isEdit && opts.params.size === 'auto'
        ? 'auto'
        : await mapFalImageSize(opts.params.size),
...
function readFalImageValue(
  ...
  if (typeof record.b64_json === 'string')
    return normalizeBase64Image(record.b64_json, fallbackMime)
  if (typeof record.base64 === 'string')
    return normalizeBase64Image(record.base64, fallbackMime)
  if (typeof record.data === 'string')
    return normalizeBase64Image(record.data, fallbackMime)
  return null
}
...
> {
  ...
  try {
    for (const candidate of candidates) {
      ...
      results.push({
        image: isHttpUrl(value)
          ? await fetchImageUrlAsDataUrl(value, fallbackMime, signal)
          : value,
...
async function parseFalResult(
  ...
  const imageResults = await parseFalImageResults(
    payload,
    mime,
    customBaseUrlLabel,
    signal
  ...
  const rawImageUrls = imageResults
    .map((r) => r.rawImageUrl)
...
export async function getFalQueuedImageResult(
  ...
  return parseFalResult(
    result.data as FalApiResponse,
    params,
    getFalCustomBaseUrlLabel(profile)
  )
}

export async function callFalAiImageApi(
  opts: CallApiOptions,
  profile: ApiProfile
): Promise<CallApiResult> {
  try {

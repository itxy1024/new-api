...
const task = (id: string, patch: Partial<TaskRecord> = {}): TaskRecord => ({
  ...
  ...(patch.agentBatchCallId
    ? { agentBatchCallId: patch.agentBatchCallId }
    : {}),
})

const blockOrder = (blocks: AgentAssistantBlock[]) =>
  blocks.map((block) => {
...
describe('agent assistant blocks', () => {
  it('projects normalized external message content without throwing', () => {
    ...
    expect(() =>
      getAgentAssistantBlocks(currentRound, [], [], true)
    ).not.toThrow()
  ...
  it('preserves response output order', () => {
    ...
    const blocks = getAgentAssistantBlocks(
      currentRound,
      getRoundTaskSlots(currentRound, [imageTask]),
      [imageTask],
      true
    )
  ...
  it('keeps deleted task placeholders in round slot order', () => {
    ...
    expect(
      getAgentAssistantBlocks(currentRound, slots, [liveTask], true).map(
        (block) =>
          block.type === 'image-task'
            ? block.task.id
            : block.type === 'deleted-image-task'
              ? block.taskId
              : block.type
      )
  ...
  it('keeps a deleted generate_image task between surrounding text', () => {
      ...
      responseOutput: [
        {
          type: 'message',
          id: 'message-before',
          content: [{ type: 'output_text', text: '生成前' }],
        },
        {
          type: 'function_call',
          name: 'generate_image',
        ...
        { type: 'function_call_output', call_id: 'image-call-1', output: '{}' },
        {
          type: 'message',
          id: 'message-after',
          content: [{ type: 'output_text', text: '生成后' }],
        },
      ],
    })
    ...
    const blocks = getAgentAssistantBlocks(
      currentRound,
      getRoundTaskSlots(currentRound, []),
      [],
      true
    )
    ...
    expect(
      blocks.find((block) => block.type === 'deleted-image-task')
    ).toMatchObject({ taskId: 'task-deleted' })
  })
  ...
  it('groups consecutive searches and gives every rendered block a stable unique key', () => {
    const currentRound = round({
      responseOutput: [
        ...
        },
        {
          type: 'message',
          id: 'duplicate-id',
          content: [{ type: 'output_text', text: '中间文本' }],
        },
        {
          type: 'web_search_call',
          id: 'search-3',
        ...
        },
        {
          type: 'message',
          id: 'duplicate-id',
          content: [{ type: 'output_text', text: '末尾文本' }],
        },
      ],
    })
    ...
    expect(
      getAgentAssistantBlocks(currentRound, [], [], true).map(
        (block) => block.key
      )
    ).toEqual(keys)
  })
  ...
  it('appends unmatched tasks and de-duplicates repeated calls and task slots', () => {
    const matchedTask = task('task-matched', {
      agentToolCallId: 'image-call-1',
    })
    const unmatchedTask = task('task-unmatched', {
      agentToolCallId: 'missing-call',
    })
    const currentRound = round({
    ...
    const blocks = getAgentAssistantBlocks(
      currentRound,
      getRoundTaskSlots(currentRound, tasks),
      tasks,
      true
    )
    ...
    expect(
      blocks.map((block) =>
        block.type === 'image-task' ? block.task.id : block.type
      )
    ).toEqual([matchedTask.id, 'text', unmatchedTask.id])
  })
  ...
  it.each([
    ...
    'projects partial batch slots in original order: $name',
    ({ taskIds, expected }) => {
      ...
      const blocks = getAgentAssistantBlocks(
        currentRound,
        getRoundTaskSlots(currentRound, [liveTask]),
        [liveTask],
        true
      )
  ...
  it('marks running web searches as stopped when the round is interrupted', () => {
    ...
    const runningBlocks = getAgentAssistantBlocks(
      round({ status: 'running', finishedAt: null, responseOutput: output }),
      [],
      [],
      false
    )
    const stoppedBlocks = getAgentAssistantBlocks(
      round({ status: 'error', error: '已停止生成。', responseOutput: output }),
      [],
      [],
      false
    )

    expect(runningBlocks[0]).toMatchObject({
      type: 'web-search',
      status: { text: '正在搜索网页', completed: false },
    })
    expect(stoppedBlocks[0]).toMatchObject({
      type: 'web-search',
      status: { text: '已停止搜索网页', completed: true },
    })
  })
  ...
  it('shows interrupted batch parameter collection as stopped', () => {
    ...
    expect(
      getAgentAssistantBlocks(currentRound, [], [], false)[0]
    ).toMatchObject({
      type: 'batch-params',
  ...
  it('copies ordered block text while retaining the fallback for text-only output', () => {
    ...
    expect(getAgentAssistantCopyContent('回退文本', mixedBlocks)).toBe(
      '第一段\n\n第二段'
    )
    expect(
      getAgentAssistantCopyContent('保留原始格式', [
        { type: 'text', key: 'text:fallback' },
      ])
    ).toBe('保留原始格式')
  })
})

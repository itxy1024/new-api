...
describe('prompt image mentions', () => {
  it('detects @ query after the cursor', () => {
    expect(getAtImageQuery('参考 @图', 5, images)).toEqual({
      start: 3,
      query: '图',
    })
  })
  ...
  it('keeps a completed image mention query selectable', () => {
    expect(getAtImageQuery('参考 @图2', 6, images)).toEqual({
      start: 3,
      query: '图2',
    })
  })
  ...
  it('splits selected agent round image mentions for tag rendering', () => {
    expect(
      getPromptMentionParts(
        `用${getSelectedTextMentionLabel('@第2轮图4')}生成`,
        images
      )
    ).toEqual([
      { type: 'text', text: '用' },
      {
        type: 'mention',
        text: '@第2轮图4',
        mentionText: getSelectedTextMentionLabel('@第2轮图4'),
      },
      { type: 'text', text: '生成' },
    ])
  })
  ...
  describe('replaceImageMentionsForApi', () => {
    it('replaces single mention', () => {
      expect(
        replaceImageMentionsForApi(`把 ${getSelectedImageMentionLabel(0)} 变蓝`)
      ).toBe('把 [image 1] 变蓝')
    })

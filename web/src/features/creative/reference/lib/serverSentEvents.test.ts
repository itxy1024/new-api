...
describe('serverSentEvents', () => {
  ...
  it('rejects event errors before invoking the callback', async () => {
    ...
    const response = new Response(
      new ReadableStream<Uint8Array>({
        start(controller) {
          controller.enqueue(
            new TextEncoder().encode('data: {"error":"生成失败"}\n\n')
          )
          controller.close()
        },
      })
    ...
    await expect(
      readJsonServerSentEvents(response, onEvent, {
        getEventErrorMessage: (event) =>
  ...
  it('formats invalid JSON errors at the caller boundary', async () => {
    ...
    await expect(
      readJsonServerSentEvents(response, vi.fn(), {
  ...
  it('rejects streams without data blocks', async () => {
    ...
    await expect(readJsonServerSentEvents(response, vi.fn())).rejects.toThrow(

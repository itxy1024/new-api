...
type TaskLifecyclePatch = Pick<
  TaskRecord,
  'status' | 'error' | 'finishedAt' | 'elapsed'
>
type ActualParams = Partial<TaskParams>
type ImageSize = { width?: number; height?: number }

export function createTaskDonePatch(
...
export function markInterruptedOpenAIRunningTasks(
  ...
  const updatedTasks = tasks.map((task) => {
    ...
    if (task.status !== 'running' || !isOpenAITask || task.customTaskId)
      return task
...
export function hasActualParams(
  params: ActualParams | undefined
): params is ActualParams {
...
export function mapActualParamsByImage(
  ...
  const mapped = paramsList?.reduce<Record<string, ActualParams>>(
    (acc, params, index) => {
...
export function addImageSizeParam(
  params: ActualParams | undefined,
...
export function deriveAgentImageActualParams(
  params: ActualParams | undefined,
  size: ImageSize | undefined
): ActualParams {
  return {
...
export function deriveGalleryActualParams(
  ...
  if (provider === 'fal' || isAsyncCustomTask)
    return firstActualParams(paramsList)
...
export function mapRevisedPromptsByImage(
  ...
  const mapped = revisedPrompts?.reduce<Record<string, string>>(
    (acc, prompt, index) => {
      const imgId = outputIds[index]

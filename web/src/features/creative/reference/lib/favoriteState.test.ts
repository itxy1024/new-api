...
describe('favorite collection normalization', () => {
  ...
  it('ensures and resolves the default collection with explicit null preserved', () => {
    expect(ensureDefaultFavoriteCollection([], 10)).toEqual([
      createDefaultFavoriteCollection(10),
    ])
    expect(ensureDefaultFavoriteCollection([collectionA], 10)).toEqual([
      collectionA,
    ])
    expect(
      resolveDefaultFavoriteCollectionId([collectionA, collectionB], null)
    ).toBeNull()
    expect(
      resolveDefaultFavoriteCollectionId(
    ...
    expect(
      resolveDefaultFavoriteCollectionId([collectionA, collectionB], 'missing')
    ).toBe(collectionA.id)
    expect(
      resolveDefaultFavoriteCollectionId(
        [createDefaultFavoriteCollection(10), collectionA],
        'missing'
      )
    ).toBe(DEFAULT_FAVORITE_COLLECTION_ID)
  })

  it('merges imports by ID while retaining collections with equal names', () => {
    const result = mergeFavoriteCollections(
      [collectionA],
      [
    ...
    expect(mergeFavoriteCollections(current, 'invalid', 10).collections).toBe(
      current
    )
  })
})
...
describe('task favorite compatibility', () => {
  it('normalizes IDs and maps legacy isFavorite through an explicit default ID', () => {
    ...
    expect(
      getTaskFavoriteCollectionIds(task({ isFavorite: true }), collectionA.id)
    ).toEqual([collectionA.id])
    expect(
      getTaskFavoriteCollectionIds(task({ isFavorite: true }), null)
    ).toEqual([])
    expect(
      getTaskFavoriteCollectionIds(
        task({ isFavorite: false, favoriteCollectionIds: [collectionB.id] }),
        collectionA.id
      )
  ...
  it('keeps isFavorite and collection IDs synchronized in task patches', () => {
    ...
    expect(
      normalizeFavoritePatch(
        favoriteTask,
        {
          favoriteCollectionIds: [collectionA.id, collectionA.id],
          isFavorite: false,
        },
        collectionB.id
      )
    ...
    expect(
      normalizeFavoritePatch(task(), { isFavorite: true }, collectionA.id)
    ).toEqual({
      isFavorite: true,
      favoriteCollectionIds: [collectionA.id],
    })
  ...
  it('normalizes loaded tasks and creates a named default for orphaned favorites', () => {
    ...
    const result = normalizeLoadedFavoriteState(
      [unchanged, legacy, invalid],
      [collectionA],
      collectionA.id,
      10
    )
  ...
  it('uses an existing collection named 默认 for legacy tasks', () => {
    ...
    expect(result.tasks[0]).toMatchObject({
      isFavorite: true,
      favoriteCollectionIds: [namedDefault.id],
    })
  })
})

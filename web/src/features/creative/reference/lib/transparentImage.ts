...
export function getTransparentRequestParams(params: TaskParams): TaskParams {
  return {
    ...
    output_compression:
      params.output_format === 'webp' ? params.output_compression : null,
    transparent_output: true,
  }
}

export function createTransparentOutputMeta(
  prompt: string
): TransparentOutputMeta {
  return {
    transparentOutput: true,
...
export async function removeKeyedBackgroundFromDataUrl(
  ...
  const effectiveKeyColor =
    keyColor ||
    detectKeyColorFromPixels(pixels.data, canvas.width, canvas.height)
  ...
  const quality =
    outputFormat === 'webp' && outputCompression != null
...
export function detectKeyColorFromPixels(
  ...
  for (const index of borderIndices) {
    ...
    const greenDist = Math.sqrt(
      (r - greenRgb.r) ** 2 + (g - greenRgb.g) ** 2 + (b - greenRgb.b) ** 2
    )
    const magentaDist = Math.sqrt(
      (r - magentaRgb.r) ** 2 +
        (g - magentaRgb.g) ** 2 +
        (b - magentaRgb.b) ** 2
    )
...
function addInteriorKeyColorIslands(
  ...
  for (let seed = 0; seed < pixelCount; seed += 1) {
    ...
    const enqueueNeighbor = (neighborIndex: number) => {
      if (neighborIndex < 0 || mask[neighborIndex] || visited[neighborIndex])
        return
...
function writeTransparentPixels(
  ...
  const distanceToBackground = computeDistanceToBackground(
    mask,
    width,
    height,
    4
  ...
  for (let index = 0; index < pixelCount; index += 1) {
    ...
    if (mask[index]) {
      ...
      if (distance > 0) {
        const transparency = getEdgeTransparency(
          red,
          green,
          blue,
          confidence,
        ...
        alpha = Math.max(
          alpha,
          distance === 1 ? 48 : distance === 2 ? 128 : 196
        )
      } else {
...
function addDistanceNeighbor(
  ...
  if (neighborIndex < 0 || mask[neighborIndex] || distance[neighborIndex] !== 0)
    return
  distance[neighborIndex] = currentDistance + 1
  nextFrontier.push(neighborIndex)
}

function getBackgroundConfidence(
  data: Uint8ClampedArray,
  index: number,
  keyRgb: Rgb
) {
...
function removeColorSpill(
  ...
  const backgroundMix = clamp01(
    Math.max(
      (255 - alpha) / 255,
      ((confidence - 0.1) / 0.9) * edgeStrength,
      spillMix
    )
  )
...
function getKeyColorRgb(keyColor: string): Rgb {
  const rgb =
    KEY_COLOR_RGB[keyColor.toUpperCase() as keyof typeof KEY_COLOR_RGB]

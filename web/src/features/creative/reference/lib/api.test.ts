...
describe('API error hints', () => {
  it.each([false, true])(
    'uses the transparent background hint when streaming is %s',
    (streamImages) => {
...
describe('callImageApi', () => {
  ...
  it('requests a transparent background from the Images edit API', async () => {
    ...
    const [, init] = fetchMock.mock.calls.find(
      ([, request]) =>
        (request as RequestInit | undefined)?.body instanceof FormData
    )!
  ...
  it('streams Images API partial images and resolves the final completed image', async () => {
    ...
    const result = await callImageApi({
      ...
      params: { ...DEFAULT_PARAMS },
      inputImageDataUrls: [],
      onPartialImage: (partial: { image: string }) =>
        partialImages.push(partial.image),
    } as any)
  ...
  it('suggests disabling streaming when a streaming request fails', async () => {
    vi.spyOn(globalThis, 'fetch').mockResolvedValue(
      new Response("invalid character ':' looking for beginning of value", {
        status: 400,
  ...
  it('splits Images API streaming into concurrent single-image requests when n is greater than 1', async () => {
    ...
    const result = await callImageApi({
      ...
      inputImageDataUrls: [],
      onPartialImage: (partial: { image: string; requestIndex?: number }) =>
    ...
    expect(partials.map((partial) => partial.requestIndex).sort()).toEqual([
      0, 1,
    ])
  ...
  it('keeps successful Images API concurrent results when one request fails', async () => {
    ...
    expect(result.failedRequests).toEqual([
      { requestIndex: 1, error: 'Failed to fetch' },
    ])
  ...
  it('keeps successful Responses API concurrent results when one request fails', async () => {
    const fetchMock = vi
      ...
      .mockImplementation(async () => {
        ...
        return new Response(
          JSON.stringify({
            output: [
              { type: 'image_generation_call', result: `aW1hZ2Ut${callIndex}` },
            ],
          }),
          {
            status: 200,
    ...
    expect(result.failedRequests).toEqual([
      { requestIndex: 2, error: 'Failed to fetch' },
    ])
    expect(result.actualParams).toMatchObject({ n: 2 })
  })
  ...
  it('keeps successful Codex CLI sync custom results when one request fails', async () => {
    const fetchMock = vi
      ...
      .mockImplementation(async () => {
        ...
        if (callIndex === 2) throw new TypeError('Failed to fetch')
        return new Response(
          JSON.stringify({ data: [{ b64_json: `aW1hZ2Ut${callIndex}` }] }),
          {
            status: 200,
            headers: { 'Content-Type': 'application/json' },
          }
  ...
  it('splits a sync edit mapping when the same custom provider has async generation', async () => {
    ...
    const submitCalls = fetchMock.mock.calls.filter(([url]) =>
      String(url).startsWith('https://api.example.com/')
    )
  ...
  it('uses the built-in sub2api async endpoints and result mapping', async () => {
    ...
    expect(fetchMock.mock.calls[0][0]).toBe(
      'https://sub2api.example.com/v1/images/generations/async'
    )
    expect(
    ...
    expect(fetchMock.mock.calls[1][0]).toBe(
      'https://sub2api.example.com/v1/images/tasks/imgtask-1'
    )

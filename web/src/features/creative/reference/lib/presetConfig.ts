...
const RAW_SHOW_PRESET_CONFIG_ONLY = readRuntimeEnv(
  import.meta.env.VITE_SHOW_PRESET_CONFIG_ONLY
)
const SHOW_PRESET_CONFIG_ONLY =
  (RAW_SHOW_PRESET_CONFIG_ONLY ||
    readRuntimeEnv(import.meta.env.VITE_SHOW_DEFAULT_CONFIG_ONLY)) === 'true'
const LOCK_PRESET_CONFIG_PARAMS =
  readRuntimeEnv(import.meta.env.VITE_LOCK_PRESET_CONFIG_PARAMS) === 'true'
const PREVENT_PRESET_CONFIG_DELETION =
  readRuntimeEnv(import.meta.env.VITE_PREVENT_PRESET_CONFIG_DELETION) === 'true'
...
export function setPresetConfig(
  settings:
    | (Pick<AppSettings, 'customProviders' | 'profiles'> & {
  ...
  defaultPresetProfileId =
    presetProfiles.length === 1
      ? presetProfiles[0].id
      : (presetProfiles.find((profile) => profile.isDefault === true)?.id ??
        null)
}
...
export function getDefaultPresetBaseUrl() {
  const profile = presetProfiles.find(
    (profile) => profile.id === defaultPresetProfileId
  )
...
export function isPresetConfigDeletionPrevented() {
  return (
    (PREVENT_PRESET_CONFIG_DELETION || SHOW_PRESET_CONFIG_ONLY) &&
...
export function isPresetProviderDeletionPrevented(
  id: string,
  profiles: ApiProfile[]
) {
  ...
  return profiles.some(
    (profile) => profile.provider === id && isPresetProfileLocked(profile.id)
  )
}
...
export function enforcePresetConfigPolicy(
  ...
  const presetProfilesById = new Map(
    presetProfiles.map((profile) => [profile.id, profile])
  )
  const presetProvidersById = new Map(
    presetProviders.map((provider) => [provider.id, provider])
  )
  const profiles = settings.profiles.map((profile) => {
    const preset = presetProfilesById.get(profile.id)
    if (!preset)
      return profile.isDefault ? { ...profile, isDefault: undefined } : profile
    return {
  ...
  for (const provider of presetProviders) {
    ...
    if (!customProviders.some((item) => item.id === provider.id))
      customProviders.push(provider)
  }
  const activeProfileId =
    presetConfigOnly && !profileIds.has(settings.activeProfileId)
      ? (defaultPresetProfileId ??
        presetProfiles[0]?.id ??
  ...
  const agentTextProfileId =
    ...
    (!settings.agentTextProfileId ||
      !profileIds.has(settings.agentTextProfileId))
      ? (presetProfiles.find(
          (profile) =>
            profile.provider === 'openai' && profile.apiMode === 'responses'
        )?.id ?? null)
      : settings.agentTextProfileId
  const agentImageProfileId =
    presetConfigOnly &&
    (!settings.agentImageProfileId ||
      !profileIds.has(settings.agentImageProfileId))

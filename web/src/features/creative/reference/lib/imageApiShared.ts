...
export interface CallApiOptions {
  ...
  prompt: string
  params: TaskParams
  /** 任务提交时锁定的 NewAPI Key/模型，避免执行期间切换配置影响旧任务 */
  newApiSelection?: { keyId: number; model: string; group: string }
...
export function getResponsesImageResultBase64(
...
function assertMaxBytes(label: string, bytes: number, maxBytes: number) {
  if (bytes > maxBytes) {
    throw new Error(
      `${label}过大：${formatMiB(bytes)}，上限为 ${formatMiB(maxBytes)}`
    )
  }
}
...
export const STREAMING_UNSUPPORTED_HINT =
  '提示：当前使用的 API 可能不支持流式传输，请尝试关闭「流式传输」功能。'
export const STREAMING_FORMAT_HINT =
  '提示：API 返回了无法解析的流式数据格式，请尝试关闭「流式传输」功能。'
export const TRANSPARENT_BACKGROUND_UNSUPPORTED_HINT =
...
export function maybeAppendTransparentBackgroundHint(message: string): string {
  if (
    !/transparent background is not supported for this model\.?/i.test(message)
  )
    return message
...
/** 排除明确与流式无关的状态码后追加提示 */
export function maybeAppendStreamingHint(
  message: string,
  status: number,
  ...
  try {
    ...
    if (err instanceof TypeError) {
      ...
      if (typeof navigator !== 'undefined' && navigator.onLine === false) {
        throw new Error(
          `图片链接下载失败（网络不可用）。${IMAGE_FETCH_CORS_HINT}`
        )
      }
      throw new Error(

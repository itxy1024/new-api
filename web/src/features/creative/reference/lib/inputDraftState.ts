...
type ActiveInputDraftState = AgentInputDraftState &
  Pick<GalleryInputDraftState, 'galleryInputDraft'>
...
export function normalizeAgentInputDrafts(
  value: unknown,
  conversations: Pick<AgentConversation, 'id'>[]
...
export function updateInputDraftImages(
  ...
  options: {
    ...
    clearMissingMask?: boolean
  } = {}
) {
  const shouldClearMask =
    options.clearMissingMask !== false &&
    Boolean(draft.maskDraft) &&
    !inputImages.some((img) => img.id === draft.maskDraft?.targetImageId)
  return {
    inputImages,
...
export function getPersistableAgentInputDrafts(
  state: AgentInputDraftState & {
    agentConversations: Pick<AgentConversation, 'id'>[]
  }
) {

...
describe('preset config policy', () => {
  it('exposes every current preset profile in preset-only mode', async () => {
    ...
    const { createDefaultFalProfile, createDefaultOpenAIProfile } =
    ...
    expect(policy.getPresetProfileIds()).toEqual(
      new Set(['preset-a', 'preset-b'])
    )
    ...
    policy.setPresetConfig({
      ...
      profiles: [
        createDefaultOpenAIProfile({
          id: 'preset-a',
          description: '使用 [说明](https://example.com)',
        }),
      ],
    })

    expect(policy.getPresetProfileDescription('preset-a')).toBe(
      '使用 [说明](https://example.com)'
    )
  ...
  it('locks preset parameters and providers except API keys without preventing profile deletion', async () => {
    ...
    const user = createDefaultFalProfile({
      id: 'user-profile',
      model: 'user-model',
    })
    const provider = {
    ...
    const removed = createDefaultOpenAIProfile({
      id: 'preset-removed',
      provider: provider.id,
    })
    policy.setPresetConfig({
      customProviders: [provider],
      profiles: [source, removed],
    })
    ...
    expect(enforced.profiles[1]).toMatchObject({
      id: user.id,
      model: 'user-model',
    })
    expect(enforced.profiles.some((profile) => profile.id === removed.id)).toBe(
      false
    )
    ...
    expect(
      policy.isPresetProviderDeletionPrevented(provider.id, enforced.profiles)
    ).toBe(true)
    expect(
      policy.isPresetProviderDeletionPrevented(
        'user-provider',
        enforced.profiles
      )
    ).toBe(false)
  ...
  it('allows removed presets to stay deleted by default', async () => {
    ...
    const policy = await import('./presetConfig')
    const presetA = createDefaultOpenAIProfile({
      id: 'preset-a',
      isDefault: true,
    })
    ...
    expect(enforced.profiles.map((profile) => profile.id)).toEqual([
      'preset-a',
      'user-profile',
    ])
  })
  ...
  it('preserves dismissed provider IDs while deletion is prevented', async () => {
    vi.stubEnv('VITE_PREVENT_PRESET_CONFIG_DELETION', 'true')
    const { createDefaultOpenAIProfile, DEFAULT_SETTINGS, normalizeSettings } =
      await import('./apiProfiles')
    const policy = await import('./presetConfig')
  ...
  it('locks preset parameters without restoring their deployment order', async () => {
    ...
    const presetA = createDefaultOpenAIProfile({
      id: 'preset-a',
      isDefault: true,
      model: 'preset-a-model',
    })
    const presetB = createDefaultFalProfile({
      id: 'preset-b',
      model: 'preset-b-model',
    })
    const user = createDefaultOpenAIProfile({ id: 'user-profile' })
    policy.setPresetConfig({
      customProviders: [],
  ...
  it('restores a dismissed provider while a current locked preset profile references it', async () => {
    vi.stubEnv('VITE_LOCK_PRESET_CONFIG_PARAMS', 'true')
    const { createDefaultOpenAIProfile, normalizeSettings } =
      await import('./apiProfiles')
    const policy = await import('./presetConfig')
    const provider = {
      id: 'preset-provider',
    ...
    const deleted = normalizeSettings({
      customProviders: [],
      profiles: [
        { ...source, provider: 'openai', model: 'local-openai-model' },
      ],
    })

    expect(
      policy.isPresetProviderDeletionPrevented(provider.id, [source])
    ).toBe(true)

    const enforced = policy.enforcePresetConfigPolicy(deleted, {
  ...
  it('allows a dismissed provider to stay deleted after its only preset profile is deleted', async () => {
    ...
    expect(policy.isPresetProviderDeletionPrevented(provider.id, [])).toBe(
      false
    )
  ...
  it('keeps a shared provider protected while another current locked preset profile references it', async () => {
    ...
    expect(
      policy.isPresetProviderDeletionPrevented(provider.id, [profiles[1]])
    ).toBe(true)
  })

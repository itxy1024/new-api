...
describe('contentEditable mentions', () => {
  ...
  it('round-trips cursor offsets around a mention', () => {
    ...
    el.innerHTML =
      '前<span contenteditable="false" class="mention-tag">@图1</span>后'
  ...
  it('clamps Ctrl+A boundaries outside the editor', () => {
    document.body.innerHTML =
      '<p>前置</p><div contenteditable="true">正文</div><p>后置</p>'
  ...
  it('uses data-mention-text when reading plain text', () => {
    const el = document.createElement('div')
    el.innerHTML =
      '前<span class="mention-tag" data-mention-text="\u2063@图1\u2064">显示文本</span>后'
  ...
  it('marks only mentions intersecting the selection as selected', () => {
    ...
    el.contentEditable = 'true'
    el.innerHTML =
      '<span contenteditable="false" class="mention-tag">@图1</span>中<span contenteditable="false" class="mention-tag selected">@图2</span>'
    document.body.append(el)

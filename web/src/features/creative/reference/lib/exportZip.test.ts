...
describe('exportZip', () => {
  it('builds and reads backup zip entries without changing manifest shape', async () => {
    ...
    expect(parsed.manifest.exportedAt).toBe(
    ...
    expect(parsed.manifest.imageFiles?.['img-2']?.path).toBe(
      'images/task-task-1.png'
    )
    expect(parsed.manifest.imageFiles?.['img-3']?.path).toBe(
      'images/task-task-1-partial.png'
    )
    ...
    expect(
      readExportZipFileAsDataUrl(parsed.files, 'images/task-task-1-input.png')
    ).toBe(images[0].dataUrl)
    expect(
      readExportZipFileAsDataUrl(parsed.files, 'images/task-task-1.png')
    ).toBe(images[1].dataUrl)
    expect(
      readExportZipFileAsDataUrl(parsed.files, 'images/task-task-1-partial.png')
    ).toBe(images[2].dataUrl)
    expect(
      readExportZipFileAsDataUrl(
        parsed.files,
        'thumbnails/task-task-1-input.jpeg'
      )
    ).toBe(thumbnail.thumbnailDataUrl)
  })
  ...
  it('splits all stored images without dropping images that are not referenced by tasks', async () => {
    ...
    const plan = getExportZipPlan(
      ...
      images.map((image) => ({
        id: image.id,
        bytes: getExportImageEstimatedBytes(image),
      })),
    ...
    expect(manifests[0].backupPart).toEqual({
      id: '1700000001000',
      index: 1,
      total: plan.length,
    })
    ...
    expect(
      manifests
        .flatMap((manifest) => Object.keys(manifest.imageFiles ?? {}))
        .sort()
    ).toEqual(['agent-only-image', 'img-1'])
    expect(
      manifests.find((manifest) => manifest.imageFiles?.['img-1'])
        ?.imageFiles?.['img-1']
    ).toMatchObject({
  ...
  it('splits task metadata across parts instead of requiring it to fit in the first part', () => {
    ...
    const plan = getExportZipPlan(params, [], {
      maxBytes: 1_800_000,
      partBytes: 1_400_000,
    })

    expect(plan.length).toBeGreaterThan(1)
    expect(plan.flatMap((part) => part.tasks).map((task) => task.id)).toEqual([
      'task-1',
      'task-2',
    ])
  })
  ...
  it('always keeps config-only exports in one part', () => {
    const plan = getExportZipPlan(
      ...
      [{ id: 'ignored-image', bytes: 2_000_000 }],
      { maxBytes: 1_800_000, partBytes: 1_400_000 }
    )

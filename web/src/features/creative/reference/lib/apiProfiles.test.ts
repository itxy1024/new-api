...
describe('normalizeApiProfile', () => {
  it('uses provider defaults and preserves explicit transparent background methods', () => {
    ...
    expect(
      normalizeApiProfile({ transparentBackgroundMethod: 'local' })
    ...
    expect(
      normalizeApiProfile({ transparentBackgroundMethod: 'invalid' })
        .transparentBackgroundMethod
    ).toBe('api')
    expect(
      normalizeApiProfile({ provider: 'fal' }).transparentBackgroundMethod
    ).toBe('local')
    expect(
      normalizeApiProfile({
        provider: 'fal',
    ...
    expect(
      normalizeApiProfile({
        provider: 'fal',
        transparentBackgroundMethod: 'invalid',
      }).transparentBackgroundMethod
    ).toBe('local')
  })
})
...
describe('default API URL env', () => {
  it('applies shared URL params from VITE_DEFAULT_API_URL to the default profile', async () => {
    ...
    vi.stubEnv(
      ...
      'https://app.example.com/?apiUrl=https%3A%2F%2Fapi.example.com&apiMode=responses&model=test-image-model&profileName=URL%20Profile&reasoningEffort=xhigh&codexCli=true&streamImages=true&streamPartialImages=3&transparentBackgroundMethod=local'
    )

    const { DEFAULT_SETTINGS, createDefaultOpenAIProfile } =
  ...
  it('enables preset-only mode for an embedded config', async () => {
    ...
    const { isPresetConfigOnlyEnabled, setPresetConfig } =
...
describe('mergeImportedSettings', () => {
  ...
  it('preserves an imported custom profile ID when its connection matches an existing profile', () => {
    ...
    const match = findEquivalentApiProfile(
      merged,
      imported.profiles[0],
  ...
  it('does not replace existing custom providers when only the default profile remains', () => {
    ...
    expect(merged.customProviders.map((provider) => provider.id)).toEqual([
      'custom-existing',
      'custom-imported',
    ])
    expect(merged.profiles).toHaveLength(2)
  ...
  it('appends imported custom providers and keeps imported custom profile references', () => {
    ...
    expect(merged.customProviders[0]).toMatchObject({
      id: 'custom-json',
      name: 'Custom JSON',
    })
    expect(merged.profiles).toHaveLength(2)
  ...
  it('preserves distinct provider and profile IDs with identical content', () => {
    ...
    expect(merged.profiles.map((profile) => profile.provider)).toEqual([
      providerA.id,
      providerB.id,
    ])
  })
...
describe('mergePresetImportedSettings', () => {
  ...
  it('applies each deployment change once and then keeps later local edits', () => {
    const initial = mergeDefaultImportedSettings(
      DEFAULT_SETTINGS,
      createConfig('model-v1', 'old/generate')
    )
    ...
    const repeated = mergeDefaultImportedSettings(merged, nextConfig, {
      previousPresetConfig: result.presetConfig,
    })
    ...
    const changedAgain = mergeDefaultImportedSettings(
      repeated.settings,
      lastConfig,
      { previousPresetConfig: repeated.presetConfig }
    )

    expect(merged.activeProfileId).toBe('source-profile')
    expect(merged.profiles.map((profile) => profile.model)).toEqual([
      'later-local-model',
      DEFAULT_FAL_MODEL,
    ])
    expect(merged.profiles[0]).toMatchObject({
    ...
    expect(repeated.settings.customProviders[0]).toMatchObject({
      id: 'source-provider',
      submit: { path: 'later-local/generate' },
    })
  ...
  it('updates matching preset parameters when parameters are locked', () => {
    const current = mergeDefaultImportedSettings(
      DEFAULT_SETTINGS,
      createConfig('model-v1', 'old/generate')
    ).settings
    const merged = mergeDefaultImportedSettings(
      current,
      createConfig('model-v2', 'v2/generate'),
      {
  ...
  it('keeps the local API key while updating other locked preset parameters', () => {
    ...
    const updated = mergeDefaultImportedSettings(current, nextConfig, {
      lockPresetParams: true,
    }).settings
    nextConfig.profiles[0].apiKey = ''
    const cleared = mergeDefaultImportedSettings(updated, nextConfig, {
      lockPresetParams: true,
    }).settings
  ...
  it('preserves the saved profile order while updating locked presets by ID', () => {
    ...
    const previous = mergeDefaultImportedSettings(
      DEFAULT_SETTINGS,
      previousConfig
    ).settings
    ...
    expect(
      merged.profiles.find((profile) => profile.id === 'preset-a')?.model
    ).toBe('model-a-v2')
    expect(
      merged.profiles.find((profile) => profile.id === 'preset-b')?.model
    ).toBe('model-b-v2')
  })
  ...
  it('keeps a user API key without duplicating an unchanged deployed config', () => {
    ...
    current.profiles[0].apiKey = 'user-key'
    const result = mergeDefaultImportedSettings(
      current,
      createConfig('model-v1', 'old/generate')
    )
  ...
  it('keeps omitted profile fields when the deployed provider type is unchanged', () => {
    const previousConfig = importCustomProviderSettingsFromJson(
      JSON.stringify({
        customProviders: [
          {
            id: 'source-provider',
    ...
    const nextConfig = importCustomProviderSettingsFromJson(
      JSON.stringify({
        customProviders: [
          {
            id: 'source-provider',
  ...
  it('uses new provider defaults when a deployment changes the provider type', () => {
    const initialConfig = importCustomProviderSettingsFromJson(
      JSON.stringify({
        customProviders: [
          {
            id: 'source-provider',
  ...
  it('does not restore an explicitly empty API key while parameters are unlocked', () => {
    const previousConfig = importCustomProviderSettingsFromJson(
      JSON.stringify({
        customProviders: [
          { id: 'provider', name: 'Provider', submit: { path: 'generate' } },
        ],
        profiles: [
          {
            id: 'profile',
  ...
  it('updates every deployed entry by ID regardless of array order', () => {
    ...
    expect(
      merged.customProviders.map((provider) => ({
        id: provider.id,
        name: provider.name,
      }))
    ).toEqual([
  ...
  it('treats changed IDs as removed and newly added entries', () => {
    const current = mergeDefaultImportedSettings(
      DEFAULT_SETTINGS,
      createConfig('model-v1', 'v1')
    ).settings
    const merged = mergeDefaultImportedSettings(current, {
      customProviders: [
        { id: 'provider-v2', name: 'Provider V2', submit: { path: 'v2' } },
      ],
      profiles: [
        { id: 'profile-v2', provider: 'provider-v2', model: 'model-v2' },
      ],
    }).settings
  ...
  it('generates stable content-based IDs for ID-less preset profiles', () => {
    ...
    const first = mergeDefaultImportedSettings(
      DEFAULT_SETTINGS,
      config
    ).settings
    ...
    expect(() =>
      importCustomProviderSettingsFromJson(
        JSON.stringify({
          ...
          profiles: [
            { id: 'profile-a', provider: 'provider-missing', model: 'model-a' },
          ],
        })
      )
  ...
  it('rejects duplicate provider IDs before normalization', () => {
    ...
    ).toThrow('部署服务商「Provider B」的 id「provider-a」重复')
  })
  ...
  it('keeps the active user profile when updating the deployed config', () => {
    ...
    const userProfile = createDefaultFalProfile({
      id: 'user-profile',
      name: 'User Profile',
    })
    const current = normalizeSettings({
  ...
  it('keeps removed preset entries and reuses the same IDs when they return', () => {
    ...
    const removed = mergeDefaultImportedSettings(first, {
      customProviders: [
        { id: 'provider-a', name: 'Provider A', submit: { path: 'a-v2' } },
      ],
      profiles: [
        { id: 'profile-a', provider: 'provider-a', model: 'model-a-v2' },
      ],
    }).settings

    expect(removed.profiles[1]).toMatchObject({
      id: 'profile-b',
      model: 'model-b',
    })
    expect(removed.customProviders[1]).toMatchObject({
      id: 'provider-b',
      submit: { path: 'b' },
    })

    const restored = mergeDefaultImportedSettings(
      removed,
      {
        customProviders: [
    ...
    expect(restored.profiles[1]).toMatchObject({
      id: 'profile-b',
      model: 'model-b-v2',
    })
    expect(restored.customProviders[1]).toMatchObject({
      id: 'provider-b',
      submit: { path: 'b-v2' },
    })
  })
  ...
  it('removes an unchanged deployed preset without history when it disappears from deployment', () => {
    ...
    const retainedForHistory = mergeDefaultImportedSettings(
      current,
      nextConfig,
      {
        previousPresetConfig,
    ...
    const removedAfterHistoryCleared = mergeDefaultImportedSettings(
      retainedForHistory.settings,
      nextConfig,
      {
        previousPresetConfig: retainedForHistory.presetConfig,
      }
    ).settings
    const modified = mergeDefaultImportedSettings(
      normalizeSettings({
        ...current,
    ...
    const modifiedProvider = mergeDefaultImportedSettings(
      normalizeSettings({
        ...current,
        customProviders: current.customProviders.map((provider) =>
          provider.id === 'provider-b'
            ? { ...provider, submit: { ...provider.submit, path: 'user-b' } }
            : provider
        ),
      }),
      nextConfig,
    ...
    expect(removed.customProviders.map((provider) => provider.id)).toEqual([
      'provider-a',
    ])
    expect(
      retainedForHistory.settings.profiles.map((profile) => profile.id)
    ).toEqual(['profile-a', 'profile-b'])
    expect(
      retainedForHistory.settings.customProviders.map((provider) => provider.id)
    ).toEqual(['provider-a', 'provider-b'])
    expect(
      removedAfterHistoryCleared.profiles.map((profile) => profile.id)
    ).toEqual(['profile-a'])
    expect(
      removedAfterHistoryCleared.customProviders.map((provider) => provider.id)
    ).toEqual(['provider-a'])
    expect(modified.profiles[1]).toMatchObject({
      id: 'profile-b',
      model: 'user-model',
    })
    expect(modified.customProviders.map((provider) => provider.id)).toEqual([
      'provider-a',
      'provider-b',
    ])
    expect(modifiedProvider.profiles.map((profile) => profile.id)).toEqual([
      'profile-a',
      'profile-b',
    ])
    expect(modifiedProvider.customProviders[1]).toMatchObject({
      id: 'provider-b',
      submit: { path: 'user-b' },
    })
  })
  ...
  it('removes an untouched unlocked preset after applying a deployment update', () => {
    ...
    const updated = mergeDefaultImportedSettings(
      initial.settings,
      updatedConfig,
      {
        previousPresetConfig: initial.presetConfig,
    ...
    const removed = mergeDefaultImportedSettings(
      updated.settings,
      {
  ...
  it('does not import preset profiles dismissed by the user', () => {
    ...
    const merged = mergeDefaultImportedSettings(
      ...
      { dismissedPresetProfileIds: ['preset-a'] }
    ).settings
  ...
  it('requires exactly one default when deploying multiple profiles', () => {
    ...
    expect(() =>
      mergeDefaultImportedSettings(DEFAULT_SETTINGS, {
  ...
  it('keeps legacy fixed slots without matching them by array position', () => {
    const legacyProvider = {
      id: 'deployed-default-provider-0',
  ...
  it('keeps every existing profile when the new preset list is empty', () => {
    const current = mergeDefaultImportedSettings(
      DEFAULT_SETTINGS,
      createConfig('model-v1', 'generate')
    ).settings
    const merged = mergeDefaultImportedSettings(current, {
      customProviders: current.customProviders.map((provider) => ({
        ...provider,
...
describe('default API profile marker', () => {
  it('only recognizes the explicit default preset marker', () => {
    expect(
      getDefaultApiProfileId(
        normalizeSettings({ profiles: [{ id: DEFAULT_OPENAI_PROFILE_ID }] })
      )
    ).toBeNull()
    expect(
      getDefaultApiProfileId({
    ...
    expect(
      getDefaultApiProfileId(
        normalizeSettings({ profiles: [{ id: 'deployed-default-profile-0' }] })
      )
    ).toBeNull()
    expect(
      getDefaultApiProfileId({
        profiles: [{ id: 'deployed-default-profile-0-previous-1' }],
      })
    ).toBeNull()
  })
  ...
  it('preserves profile order when marking the deployment default', () => {
    ...
    const deployed = createDefaultOpenAIProfile({
      id: 'deployment-profile',
      isDefault: true,
    })

    expect(
      normalizeSettings({ profiles: [userProfile, builtIn] }).profiles.map(
        (profile) => profile.id
      )
    ).toEqual([userProfile.id, DEFAULT_OPENAI_PROFILE_ID])
    expect(
      normalizeSettings({
        profiles: [builtIn, userProfile, deployed],
      }).profiles.map((profile) => profile.id)
    ).toEqual([builtIn.id, userProfile.id, deployed.id])
    expect(
      normalizeSettings({ profiles: [builtIn, userProfile, deployed] })
        .profiles[2].isDefault
    ).toBe(true)
  })
})
...
describe('custom providers', () => {
  it('provides the built-in sub2api async manifest', () => {
    ...
    expect(getApiProviderLabel(settings, 'sb2api-async')).toBe(
      'sub2api（异步）'
    )
  })
  ...
  it('defaults custom profiles based on their native transparent background mapping', () => {
    const settings = normalizeSettings({
      customProviders: [
        {
          id: 'custom-local',
          name: 'Custom Local',
          submit: { path: 'generate', body: { prompt: '$prompt' } },
        },
        {
          id: 'custom-native',
          name: 'Custom Native',
      ...
      profiles: [
        {
          ...createDefaultOpenAIProfile({ id: 'local-profile' }),
          provider: 'custom-local',
          transparentBackgroundMethod: 'api',
        },
        {
          ...createDefaultOpenAIProfile({ id: 'native-profile' }),
          provider: 'custom-native',
          transparentBackgroundMethod: undefined,
        },
        {
          ...createDefaultOpenAIProfile({ id: 'partial-profile' }),
          provider: 'custom-partial',
          transparentBackgroundMethod: undefined,
        },
        {
    ...
    expect(
      settings.profiles.find((profile) => profile.id === 'local-profile')
        ?.transparentBackgroundMethod
    ).toBe('local')
    expect(
      settings.profiles.find((profile) => profile.id === 'native-profile')
        ?.transparentBackgroundMethod
    ).toBe('api')
    expect(
      settings.profiles.find((profile) => profile.id === 'partial-profile')
        ?.transparentBackgroundMethod
    ).toBe('local')
    expect(
      settings.profiles.find((profile) => profile.id === 'draft-profile')
        ?.providerDrafts?.['custom-local']?.transparentBackgroundMethod
    ).toBe('local')
    expect(
      settings.profiles.find((profile) => profile.id === 'draft-profile')
        ?.providerDrafts?.['custom-native']?.transparentBackgroundMethod
    ).toBe('api')
  })
  ...
  it('imports wrapped custom provider settings with profiles', () => {
    ...
    expect(imported.customProviders[0]).toMatchObject({
      id: 'custom-json',
      name: 'Custom JSON',
    })
...
\`\`\``)
  ...
  it('restores custom provider draft settings', () => {
    ...
    const openAIProfile = switchApiProfileProvider(
      {
        ...customProfile,
        codexCli: true,
        transparentBackgroundMethod: 'local',
    ...
    const restoredProfile = switchApiProfileProvider(
      openAIProfile,
      provider.id,
      provider
    )
  ...
  it('uses API-mode specific streaming defaults and preserves partial image count', () => {
    ...
    expect(
      createDefaultOpenAIProfile({ apiMode: 'responses' }).streamImages
    ).toBe(true)
  ...
  it('normalizes supported reasoning efforts and ignores invalid values', () => {
    ...
    const invalid = normalizeSettings({
      profiles: [
        {
          ...createDefaultOpenAIProfile({ apiMode: 'responses' }),
  ...
  it('keeps non-OpenAI providers in Images API mode when switching providers', () => {
    ...
    expect(falProfile).toMatchObject({
      provider: 'fal',
      apiMode: 'images',
      streamImages: false,
      transparentBackgroundMethod: 'local',
  ...
  it('enables Agent submit auto scroll by default', () => {
    ...
    expect(
      normalizeSettings({ agentScrollToBottomAfterSubmit: false })
  ...
  it('enables Agent math formatting prompt by default', () => {
    ...
    expect(
      normalizeSettings({ agentMathFormattingPrompt: false })
  ...
  it('disables prompt rewrite allowance by default', () => {
    ...
    expect(
      normalizeSettings({ allowPromptRewrite: true }).allowPromptRewrite
    ).toBe(true)
  })

...
const DEFAULT_API_URL_PATCH = isImportableConfigUrl(RAW_DEFAULT_API_URL)
  ...
  : parseDefaultApiUrl(
      RAW_DEFAULT_API_URL ||
        (DOCKER_DEPLOYMENT && DEFAULT_OPENAI_API_PROXY
          ? ''
...
const BUILT_IN_PROVIDER_IDS = new Set<ApiProvider>([
  'openai',
  'sb2api-async',
  'fal',
])
...
type ApiProfileProviderDraft = NonNullable<
  ApiProfile['providerDrafts']
>[ApiProvider]

function getDefaultStreamImages(
...
export function getDefaultApiProfileId(
  settings: Partial<AppSettings> | unknown
): string | null {
  const record = isRecord(settings) ? settings : {}
  const profiles = Array.isArray(record.profiles)
    ? record.profiles.filter(isRecord)
    : []
  const marked = profiles.find(
    (profile) => profile.isDefault === true && typeof profile.id === 'string'
  )
...
function normalizeZipDownloadRoutes(value: unknown) {
  ...
  return value.filter(
    (item): item is (typeof ZIP_DOWNLOAD_ROUTE_VALUES)[number] =>
...
function isCustomProviderTemplate(
  value: unknown
): value is CustomProviderTemplate {
...
function normalizeResultMapping(
  ...
  const imageUrlPaths = normalizeStringArray(
    record.imageUrlPaths,
    fallback.imageUrlPaths ?? []
  )
  const b64JsonPaths = normalizeStringArray(
    record.b64JsonPaths,
    fallback.b64JsonPaths ?? []
  )
  return {
    imageUrlPaths,
    b64JsonPaths,
...
function normalizeSubmitMapping(
  ...
  return {
    ...
    files:
      contentType === 'multipart'
        ? normalizeFileMappings(record.files, fallback.files)
        : undefined,
    taskIdPath:
      typeof record.taskIdPath === 'string' && record.taskIdPath.trim()
        ? record.taskIdPath.trim()
        : fallback.taskIdPath,
    result: normalizeResultMapping(
      record.result,
      fallback.result ?? DEFAULT_OPENAI_RESULT
    ),
  }
}
...
function legacyCustomProviderToManifest(
  ...
  if (
    record.template !== 'openai-compatible' &&
    record.template !== 'openai-compatible-async'
  )
    return null
  ...
  return {
    ...
    submit: {
      path:
        record.generationPath ?? DEFAULT_CUSTOM_PROVIDER_PATHS.generationPath,
      method: 'POST',
      contentType: 'json',
      query: isAsync
        ? (normalizeStringRecord(record.submitQuery) ?? { async: 'true' })
        : undefined,
      body: DEFAULT_GENERATE_BODY,
...
export function normalizeCustomProviderDefinitions(
  input: unknown
): CustomProviderDefinition[] {
...
export function createDefaultOpenAIProfile(
  ...
  const apiMode =
    overrides.apiMode ?? DEFAULT_API_URL_PATCH?.apiMode ?? 'images'
  const streamImages =
    overrides.streamImages ??
    DEFAULT_API_URL_PATCH?.streamImages ??
    getDefaultStreamImages('openai', apiMode)
  ...
  return {
    ...
    streamPartialImages:
      DEFAULT_API_URL_PATCH?.streamPartialImages ??
      DEFAULT_STREAM_PARTIAL_IMAGES,
    transparentBackgroundMethod:
      DEFAULT_API_URL_PATCH?.transparentBackgroundMethod ?? 'api',
    ...overrides,
    apiMode,
    streamImages,
  }
...
export function switchApiProfileProvider(
  ...
  if (customProvider) {
    ...
    const supportsNativeTransparentBackground =
      customProviderSupportsNativeTransparentBackground(customProvider)
    return {
      ...profile,
      provider: customProvider.id,
      baseUrl:
        savedDraft?.baseUrl ??
        (shouldUseOpenAIDefaults
          ? DEFAULT_BASE_URL
      ...
      streamPartialImages:
        savedDraft?.streamPartialImages ?? DEFAULT_STREAM_PARTIAL_IMAGES,
      transparentBackgroundMethod: supportsNativeTransparentBackground
        ? (savedDraft?.transparentBackgroundMethod ?? 'api')
        : 'local',
      providerDrafts,
    }
  }
  ...
  const nextStreamImages =
    savedDraft?.streamImages ??
    (profile.provider === 'openai'
      ? profile.streamImages
      : getDefaultStreamImages(provider, nextApiMode))
  const nextStreamPartialImages =
...
function normalizeProviderDraft(
  ...
  const nativeTransparentBackgroundUnavailable =
    customProviderIds.has(provider) &&
    !nativeTransparentProviderIds.has(provider)
  const transparentBackgroundMethod = nativeTransparentBackgroundUnavailable
    ? 'local'
    : 'api'
  const fallback =
    provider === 'fal'
  ...
  const apiMode =
    input.apiMode === 'responses'
      ...
      : input.apiMode === 'images'
        ...
        : undefined
  const knownProvider =
    BUILT_IN_PROVIDER_IDS.has(provider) || customProviderIds.has(provider)
  ...
  return {
    ...
    streamImages:
      typeof input.streamImages === 'boolean'
        ? input.streamImages
        : fallback.streamImages,
    streamPartialImages: normalizeStreamPartialImages(
      input.streamPartialImages,
      fallback.streamPartialImages
    ),
    transparentBackgroundMethod:
      !nativeTransparentBackgroundUnavailable &&
      (input.transparentBackgroundMethod === 'api' ||
        input.transparentBackgroundMethod === 'local')
      ...
      ([provider, draft]) =>
        [
          ...
          normalizeProviderDraft(
            ...
            provider,
            customProviderIds,
            nativeTransparentProviderIds
          ),
        ] as const
    )
    .filter(
      (entry): entry is [ApiProvider, NonNullable<ApiProfileProviderDraft>] =>
        Boolean(entry[1])
    )
...
export function normalizeApiProfile(
  ...
  const provider: ApiProvider =
    BUILT_IN_PROVIDER_IDS.has(rawProvider) || customProviderIds.has(rawProvider)
      ? rawProvider
      : 'openai'
  const apiMode: ApiMode =
    provider === 'openai' && record.apiMode === 'responses'
      ? 'responses'
      : 'images'
  ...
  return {
    ...
    provider,
    baseUrl:
      provider === 'fal' ? rawBaseUrl.trim().replace(/\/+$/, '') : rawBaseUrl,
    ...
    streamPartialImages: normalizeStreamPartialImages(
      ...
      defaults.streamPartialImages
    ),
    transparentBackgroundMethod:
      !nativeTransparentBackgroundUnavailable &&
      (record.transparentBackgroundMethod === 'api' ||
        record.transparentBackgroundMethod === 'local')
...
function validateDeploymentProviderIds(input: unknown) {
  ...
  for (const item of input) {
    ...
    if (BUILT_IN_PROVIDER_IDS.has(id))
      throw new Error(`部署服务商${label}的 id「${id}」与内置服务商冲突`)
    if (usedIds.has(id)) throw new Error(`部署服务商${label}的 id「${id}」重复`)
    if (!normalizeCustomProviderDefinition(item))
      throw new Error(`部署服务商${label}的 Manifest 无效`)
    usedIds.add(id)
  }
}

function normalizeDeploymentProfileEntries(
  input: unknown,
  customProviderIds: Set<string>,
  nativeTransparentProviderIds = new Set<string>(),
  ...
  const records = Array.isArray(input)
    ? input.flatMap((item) => {
        ...
        if (
          ...
          (!BUILT_IN_PROVIDER_IDS.has(item.provider) &&
            !customProviderIds.has(item.provider))
        ) {
          const name = typeof item.name === 'string' ? item.name.trim() : ''
          const id = typeof item.id === 'string' ? item.id.trim() : ''
  ...
  if (deploymentConfig && records.length > 1) {
    const defaultCount = records.filter(
      (record) => record.isDefault === true
    ).length
    if (defaultCount !== 1)
      throw new Error(
        '部署文件包含多个 API 配置时，必须且只能有一项设置 isDefault: true'
      )
  }
  ...
  return records.map((record) => {
    ...
    const fallback = nativeTransparentProviderIds.has(record.provider as string)
      ? { transparentBackgroundMethod: 'api' as const }
      : undefined
    if (rawId)
      return {
        source: record,
        profile: normalizeApiProfile(
          { ...record, id: rawId },
          fallback,
          customProviderIds,
          nativeTransparentProviderIds
...
export function normalizeSettings(
  ...
  const record =
    input && typeof input === 'object' ? (input as Record<string, unknown>) : {}
  const customProviders = normalizeCustomProviderDefinitions(
    record.customProviders
  )
  const customProviderIds = new Set(
    customProviders.map((provider) => provider.id)
  )
  const nativeTransparentProviderIds = new Set(
    customProviders
      .filter(customProviderSupportsNativeTransparentBackground)
      .map((provider) => provider.id)
  )
  const legacyApiMode: ApiMode =
    record.apiMode === 'responses' ? 'responses' : 'images'
  const legacyProfile = createDefaultOpenAIProfile({
    baseUrl:
      typeof record.baseUrl === 'string' ? record.baseUrl : DEFAULT_BASE_URL,
  ...
  const normalizedProfiles =
    Array.isArray(record.profiles) && record.profiles.length
      ? record.profiles.map((profile) => {
          const provider =
            isRecord(profile) && typeof profile.provider === 'string'
              ? profile.provider
              : ''
          const fallback = nativeTransparentProviderIds.has(provider)
            ? { transparentBackgroundMethod: 'api' as const }
            : undefined
      ...
      : [legacyProfile]
  const defaultProfileId = getDefaultApiProfileId({
  ...
  const agentApiConfigMode = normalizeAgentApiConfigMode(
    record.agentApiConfigMode
  )
  const firstAgentTextProfile = profiles.find(isAgentTextApiProfile)
  const agentTextProfileId =
    typeof record.agentTextProfileId === 'string' &&
    profiles.some(
      (p) => p.id === record.agentTextProfileId && isAgentTextApiProfile(p)
    )
      ? record.agentTextProfileId
      : isAgentTextApiProfile(active)
        ? active.id
        : (firstAgentTextProfile?.id ?? null)
  const agentImageProfileId =
    typeof record.agentImageProfileId === 'string' &&
    profiles.some((p) => p.id === record.agentImageProfileId)
  ...
  return {
    ...
    clearInputAfterSubmit:
      typeof record.clearInputAfterSubmit === 'boolean'
        ? record.clearInputAfterSubmit
        : false,
    persistInputOnRestart:
      typeof record.persistInputOnRestart === 'boolean'
        ? record.persistInputOnRestart
        : true,
    ...
    alwaysShowRetryButton:
      typeof record.alwaysShowRetryButton === 'boolean'
        ? record.alwaysShowRetryButton
        : false,
    allowPromptRewrite:
      typeof record.allowPromptRewrite === 'boolean'
        ? record.allowPromptRewrite
        : false,
    taskCompletionNotification:
      typeof record.taskCompletionNotification === 'boolean'
        ? record.taskCompletionNotification
        : false,
    enterSubmit:
      typeof record.enterSubmit === 'boolean' ? record.enterSubmit : false,
    zipDownloadRoutes: normalizeZipDownloadRoutes(record.zipDownloadRoutes),
    agentScrollToBottomAfterSubmit:
      typeof record.agentScrollToBottomAfterSubmit === 'boolean'
        ? record.agentScrollToBottomAfterSubmit
        : true,
    agentMaxToolRounds: normalizeAgentMaxToolRounds(record.agentMaxToolRounds),
    agentWebSearch:
      typeof record.agentWebSearch === 'boolean'
        ? record.agentWebSearch
        : false,
    agentMathFormattingPrompt:
      typeof record.agentMathFormattingPrompt === 'boolean'
        ? record.agentMathFormattingPrompt
        : true,
    agentApiConfigMode,
    agentTextProfileId,
    agentImageProfileId,
...
export function getAgentTextApiProfile(
  settings: Partial<AppSettings> | unknown
): ApiProfile | null {
  ...
  return (
    normalized.profiles.find(
      (profile) => profile.id === normalized.agentTextProfileId
    ) ?? null
  )
}

export function getAgentImageApiProfile(
  settings: Partial<AppSettings> | unknown
): ApiProfile | null {
  const normalized = normalizeSettings(settings)
  if (normalized.agentApiConfigMode !== 'hybrid')
    return getAgentTextApiProfile(normalized)
  return (
    normalized.profiles.find(
      (profile) => profile.id === normalized.agentImageProfileId
    ) ?? null
  )
}
...
export function isOpenAICompatibleProvider(
  ...
  return (
    provider === 'openai' ||
    Boolean(getCustomProviderDefinition(settings, provider))
  )
}
...
function validateCustomProviderTaskMappings(
  providers: CustomProviderDefinition[]
) {
  const invalid = providers.find(
    (provider) =>
      (provider.submit.taskIdPath || provider.editSubmit?.taskIdPath) &&
      !provider.poll
  )
  if (invalid)
    throw new Error(
      `自定义服务商「${invalid.name}」配置了 taskIdPath，但缺少 poll`
    )
}
...
export function importCustomProviderSettingsFromJson(
  ...
  if (Array.isArray(record.customProviders)) {
    if (options.deploymentConfig)
      validateDeploymentProviderIds(record.customProviders)
    const customProviders = normalizeCustomProviderDefinitions(
      record.customProviders
    )
    ...
    const profileEntries = normalizeDeploymentProfileEntries(
      record.profiles,
      customProviderIds,
      nativeTransparentProviderIds,
    ...
    return {
      ...
      ...(profileEntries.length
        ? {
            presetProfileFields: Object.fromEntries(
...
export function getActiveApiProfile(
  ...
  return {
    ...
    apiProxy:
      typeof record.apiProxy === 'boolean' ? record.apiProxy : profile.apiProxy,
    streamImages:
      profile.provider === 'openai' && typeof record.streamImages === 'boolean'
        ? record.streamImages
        : profile.streamImages,
    streamPartialImages: normalizeStreamPartialImages(
      record.streamPartialImages,
...
export function validateApiProfile(profile: ApiProfile): string | null {
  ...
  if (
    ...
    !shouldUseApiProxy(profile.apiProxy)
  )
    return '缺少 API URL'
...
function createImportedProfileId(
  provider: ApiProvider,
  usedIds: Set<string>
): string {
...
function createPresetProfileId(
  provider: ApiProvider,
  record: Record<string, unknown>,
  usedIds: Set<string>
): string {
...
export function findEquivalentApiProfile(
  ...
  const normalized = normalizeSettings(settings)
  const importedProvider = importedProviders.find(
    (provider) => provider.id === importedProfile.provider
  )
  const provider = importedProvider
    ? (normalized.customProviders.find(
        (provider) =>
          getCustomProviderDedupKey(provider) ===
          getCustomProviderDedupKey(importedProvider)
  ...
  const exact = normalized.profiles.find(
    (item) => getApiProfileDedupKey(item) === dedupKey
  )
  ...
  return (
    normalized.profiles.find(
      (item) => getApiProfileConnectionKey(item) === connectionKey
    ) ?? null
  )
}
...
export function mergeImportedSettings(
  ...
  const { providers: customProviders, providerIdMap } =
    mergeImportedCustomProviders(
      current.customProviders,
      imported.customProviders
    )
  const profiles = [...current.profiles]
  const hasProfileArray =
    isRecord(importedSettings) && Array.isArray(importedSettings.profiles)
  for (const importedProfile of imported.profiles) {
    const mappedProfile = providerIdMap.has(importedProfile.provider)
      ? {
          ...importedProfile,
          provider:
            providerIdMap.get(importedProfile.provider) ??
            importedProfile.provider,
        }
      : importedProfile
...
function getPresetDeploymentFields(input: unknown, id: string, source: object) {
  if (!isRecord(input) || !Array.isArray(input[id]))
    return new Set(Object.keys(source))
  return new Set(
    input[id].filter((key): key is string => typeof key === 'string')
  )
}

function mergePresetProfileSnapshot(
  previous: ApiProfile,
  imported: ApiProfile,
  fields: Set<string>
) {
...
export function mergePresetImportedSettings(
  ...
  presetConfig: Pick<AppSettings, 'customProviders' | 'profiles'>
} {
  ...
  const current = normalizeSettings(currentSettings)
  const customProviderIds = new Set(
    normalizedImported.customProviders.map((provider) => provider.id)
  )
  const nativeTransparentProviderIds = new Set(
    normalizedImported.customProviders
      .filter(customProviderSupportsNativeTransparentBackground)
      .map((provider) => provider.id)
  )
  const allSourceProfileEntries = normalizeDeploymentProfileEntries(
    importedRecord.profiles,
    customProviderIds,
    nativeTransparentProviderIds,
    true
  ).map((entry) => ({
  ...
  const sourceProfileEntries = allSourceProfileEntries.filter(
    (entry) => !dismissedIds.has(entry.profile.id)
  )
  const sourceProfiles = sourceProfileEntries.map((entry) => entry.profile)
  const sourceProfileIds = new Set(
    allSourceProfileEntries.map((entry) => entry.profile.id)
  )
  const sourceDefaultProfileId =
    allSourceProfileEntries.length === 1
      ? allSourceProfileEntries[0].profile.id
      : (allSourceProfileEntries.find((entry) => entry.isDefault)?.profile.id ??
        null)
  const replacingPristineDefault =
    sourceProfiles.length > 0 && hasOnlyDefaultProfiles(current)
  const currentProvidersById = new Map(
    current.customProviders.map((provider) => [provider.id, provider])
  )
  const previousProfilesById = new Map(
    options.previousPresetConfig?.profiles.map((profile) => [
      profile.id,
      profile,
    ]) ?? []
  )
  const previousProvidersById = new Map(
    options.previousPresetConfig?.customProviders.map((provider) => [
      provider.id,
      provider,
    ]) ?? []
  )
  const nextProviders = normalizedImported.customProviders
    .filter((provider) => !dismissedProviderIds.has(provider.id))
    .map((provider) => {
      ...
      if (!previous || JSON.stringify(provider) === JSON.stringify(previous))
        return matched
      return provider
    })
  ...
  const modifiedProviderIds = new Set(
    current.customProviders
      .filter((provider) => {
        const previous = previousProvidersById.get(provider.id)
  ...
  const removableProfileIds = new Set(
    current.profiles
      .filter((profile) => {
        ...
        if (
          !previous ||
          sourceProfileIds.has(profile.id) ||
          usedPresetProfileIds.has(profile.id) ||
          modifiedProviderIds.has(profile.provider)
        )
          return false
        return (
          JSON.stringify({ ...profile, isDefault: undefined }) ===
          JSON.stringify({ ...previous, isDefault: undefined })
        )
      })
  ...
  const nextProfiles = sourceProfileEntries.map((entry) => {
    const matched = replacingPristineDefault
      ? undefined
      : currentProfilesById.get(entry.profile.id)
    ...
    const fields = getPresetDeploymentFields(
      importedRecord.presetProfileFields,
      entry.profile.id,
      entry.source
    )
    ...
    if (options.lockPresetParams)
      return { ...importedProfile, apiKey: matched.apiKey }
    if (!previous)
      return {
        ...matched,
        baseUrl: importedProfile.baseUrl,
        isDefault: importedProfile.isDefault,
      }

    const changed: Partial<ApiProfile> = {}
    for (const key of PRESET_PROFILE_DEPLOYMENT_KEYS) {
      if (
        JSON.stringify(importedProfile[key]) !== JSON.stringify(previous[key])
      ) {
  ...
  const profiles = replacingPristineDefault
    ...
    : [
        ...current.profiles
          ...
          .map(
            (profile) =>
              nextProfilesById.get(profile.id) ??
              (profile.isDefault
                ? { ...profile, isDefault: undefined }
                : profile)
          ),
        ...nextProfiles.filter(
          (profile) => !currentProfilesById.has(profile.id)
        ),
      ]
  const retainedProviders = current.customProviders
    ...
    .filter((provider) => {
      const previous = previousProvidersById.get(provider.id)
      const unchangedRemovedPreset =
        previous && JSON.stringify(provider) === JSON.stringify(previous)
      return (
        !unchangedRemovedPreset ||
  ...
  for (const provider of options.previousPresetConfig?.customProviders ?? []) {
    if (
      !sourceProviderIds.has(provider.id) &&
      customProviders.some((item) => item.id === provider.id)
    )
      presetProviders.push(provider)
  }
  ...
  return {
    settings: normalizeSettings({
      ...
      customProviders,
      profiles,
      activeProfileId: replacingPristineDefault
        ? (sourceDefaultProfileId ?? nextProfiles[0].id)
...
export const DEFAULT_SETTINGS: AppSettings = normalizeSettings({
  ...
  streamImages:
    ...
    getDefaultStreamImages(
      'openai',
      DEFAULT_API_URL_PATCH?.apiMode ?? 'images'
    ),
  streamPartialImages:
    DEFAULT_API_URL_PATCH?.streamPartialImages ?? DEFAULT_STREAM_PARTIAL_IMAGES,
  customProviders: [],

...
function normalizeAnnotations(value: unknown) {
  ...
  return value.filter(isRecord).map((annotation) => {
    ...
    if (
      normalized.start_index !== undefined &&
      (typeof normalized.start_index !== 'number' ||
        !Number.isFinite(normalized.start_index))
    )
      delete normalized.start_index
    if (
      normalized.end_index !== undefined &&
      (typeof normalized.end_index !== 'number' ||
        !Number.isFinite(normalized.end_index))
    )
      delete normalized.end_index
    return normalized
  })
}
...
function normalizeMessageContent(value: unknown) {
  ...
  for (const part of value) {
    if (!isRecord(part) || typeof part.type !== 'string' || !part.type.trim())
      continue
...
function normalizeFunctionOutputContent(
  value: unknown
): ResponsesInputContentItem[] | null {
  if (!Array.isArray(value)) return null
  ...
  for (const part of value) {
    ...
    if (part.type === 'input_image') {
      const imageUrl =
        typeof part.image_url === 'string' && part.image_url.trim()
          ...
          : undefined
      const fileId =
        typeof part.file_id === 'string' && part.file_id.trim()
          ? part.file_id
          : undefined
    ...
    if (part.type === 'input_file') {
      const normalized: Record<string, unknown> = { ...part, type: part.type }
      for (const key of ['file_data', 'file_id', 'file_url', 'filename'])
        deleteInvalidString(normalized, key)
      const hasSource = ['file_data', 'file_id', 'file_url'].some(
        (key) =>
          typeof normalized[key] === 'string' &&
          Boolean((normalized[key] as string).trim())
      )
    ...
    if (type === 'function_call_output') {
      ...
      const output =
        typeof item.output === 'string'
          ? item.output
          : normalizeFunctionOutputContent(item.output)

...
export function getPersistableAgentConversations(
    ...
    return {
      ...
      rounds: conversation.rounds.map((round) => {
        ...
        return {
          ...
          ).map(getPersistableResponseOutputItem),
        }
      }),
    }
  })
...
export function getPersistableRawResponsePayload(rawResponsePayload?: string) {
  ...
  try {
    ...
    const output = normalizeResponsesOutputItems(payload.output).map(
      getPersistableResponseOutputItem
    )
...
function parseResponseOutputFromPayload(
  ...
  try {
    ...
    return Array.isArray(payload.output)
      ? normalizeResponsesOutputItems(payload.output)
      : null
  } catch {
    return null
  }
...
export function canonicalizeBatchFunctionCallArguments(
  output: ResponsesOutputItem[]
) {
  let changed = false
  const canonical = output.map((item) => {
    if (item.type !== 'function_call' || item.name !== 'generate_image_batch')
      return item
...
export function scrubResponseOutputForDeletedAgentTasks(
  ...
  const batchItemsByCallId = new Map<
    string,
  ...
  for (const batchCallId of new Set(
    deletedTasks
      .map((task) => task.agentBatchCallId)
      .filter((id): id is string => Boolean(id))
  )) {
    const batchTasks = round.outputTaskIds
      .map((taskId) => tasksById.get(taskId))
      .filter(
        (task): task is TaskRecord => task?.agentBatchCallId === batchCallId
      )
    batchTasksByCallId.set(batchCallId, batchTasks)
    if (
      batchTasks.length > 0 &&
      batchTasks.every((task) => deletedTaskIds.has(task.id))
    ) {
      removedFunctionCallIds.add(batchCallId)
      continue
    }
    const batchItemIds = (batchItemsByCallId.get(batchCallId) ?? []).map(
      (item) => item.id
    )
    ...
    for (let index = 0; index < batchTasks.length; index++) {
      ...
      const itemId =
        batchItemIds.length === batchTasks.length
          ? batchItemIds[index]
          : task.agentBatchItemId
  ...
  for (const item of output) {
    if (item.type === 'image_generation_call') {
      ...
      imageIndex += 1
      if (
        (item.id && deletedToolCallIds.has(item.id)) ||
        (!item.id && deletedTaskIds.has(taskId))
      ) {
        changed = true
        continue
      }
    ...
    if (
      callId &&
      removedFunctionCallIds.has(callId) &&
    ...
    if (item.type === 'function_call') {
      try {
        ...
        const args = JSON.stringify({
          ...parsed,
          images: batchItems.filter((batchItem) => !itemIds.has(batchItem.id)),
        })
    ...
    try {
      ...
      const canonicalImages =
        ...
        parsed.images.length === batchItems.length
          ? parsed.images.map((image, index) =>
              isRecord(image) ? { ...image, id: batchItems[index].id } : image
            )
          : parsed.images
...
function createAgentRecoveredToolOutputs(
  round: AgentRound,
  tasks: TaskRecord[]
) {
  ...
  for (const item of output) {
    if (
      item.type !== 'function_call' ||
      !item.call_id ||
      existingOutputCallIds.has(item.call_id)
    )
      continue
    ...
    if (item.name === 'generate_image') {
      const imageId = (() => {
        try {
          const value = JSON.parse(item.arguments ?? '{}') as Record<
            string,
            unknown
          >
      ...
      const task = tasks.find(
        (task) =>
          ...
          task.agentToolCallId === item.call_id
      )
      if (
        !task ||
        task.status === 'running' ||
    ...
    if (item.name === 'generate_image_batch') {
      ...
      const batchTasks = round.outputTaskIds
        ...
        .filter((task): task is TaskRecord =>
          Boolean(task && task.agentBatchCallId === item.call_id)
        )
      if (
        batchTasks.length < batchItems.length ||
        batchTasks.some(
...
export function createReadyAgentRecoveredToolState(
  round: AgentRound,
  tasks: TaskRecord[]
) {
  const recovered = createAgentRecoveredToolOutputs(round, tasks)
  if (recovered) return recovered
  if (!round.responseOutput?.length || round.outputTaskIds.length === 0)
    return null
  ...
  return {
    ...
    allSuccessful: roundTasks.every(
      (task) => task.status === 'done' && task.outputImages.length > 0
    ),
  }
}

export function getAgentRecoveredToolCallCount(
  output: ResponsesOutputItem[],
  tasks: TaskRecord[]
) {
  const functionOutputs = output.filter(
    (item) => item.type === 'function_call_output'
  )
  const functionCallCount = functionOutputs.reduce((count, item) => {
    if (typeof item.output !== 'string' || !item.output) return count
    try {
      const payload = JSON.parse(item.output) as {
        images?: unknown[]
        status?: string
      }
      if (Array.isArray(payload.images))
        return (
          ...
          payload.images.filter(
            (image) => isRecord(image) && image.status === 'done'
          ).length
        )
...
export function getAgentRecoveredFailureError(
  round: AgentRound,
  tasks: TaskRecord[]
) {

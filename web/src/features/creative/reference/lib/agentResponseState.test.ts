...
describe('agent response persistence', () => {
  ...
  it('cleans legacy unknown state and task payloads while dropping malformed output items', () => {
    ...
    expect(
      JSON.stringify(stripPersistedAgentConversations(legacy))
    ).not.toContain('legacy-base64')
    expect(stripPersistedAgentConversations({ rounds: [] })).toEqual({
      rounds: [],
    })
    expect(getPersistableRawResponsePayload(payload)).toBe(
      JSON.stringify(
        {
          id: 'response-a',
    ...
    expect(getPersistableRawResponsePayload('{"output":null}')).toBe(
      '{"output":null}'
    )
  })
})
...
describe('agent response input and merging', () => {
  it('removes image and web payloads, normalizes messages, and keeps only paired function items', () => {
    ...
    expect(
      sanitizeResponseOutputForInput(output, {
  ...
  it('prefers round output and otherwise reads the first valid task payload', () => {
    ...
    const tasks = [
      ...
      task('valid-output', {
        rawResponsePayload: JSON.stringify({ output: fallback }),
      }),
    ]
    ...
    expect(
      getAgentRoundResponseOutput(
        round({ outputTaskIds: tasks.map((item) => item.id) }),
        tasks
      )
    ).toEqual(fallback)
    expect(
      getAgentRoundResponseOutput(
        round({ outputTaskIds: ['missing-output'] }),
        tasks
      )
    ).toBeNull()
  })
...
describe('deleted Agent task response scrubbing', () => {
  ...
  it('removes both sides of deleted single and complete batch function calls', () => {
    ...
    const batchA = task('batch-a', {
      agentRoundId: 'round-a',
      agentToolCallId: 'image-a',
      agentBatchCallId: 'batch-call',
    })
    const batchB = task('batch-b', {
      agentRoundId: 'round-a',
      agentToolCallId: 'image-b',
      agentBatchCallId: 'batch-call',
    })
    const output: ResponsesOutputItem[] = [
      ...
      },
      {
        type: 'function_call_output',
        call_id: 'batch-call',
        output: JSON.stringify({ images: [{ id: 'a' }, { id: 'b' }] }),
      },
  ...
  it.each([
    ...
    'scrubs one historical batch occurrence with duplicate ids at index $deletedIndex',
    ({ deletedIndex, liveId, livePrompt }) => {
      ...
      const tasks = ['first prompt', 'second prompt', 'missing prompt'].map(
        (prompt, index) =>
          task(`task-${index}`, {
            prompt,
            agentRoundId: 'round-a',
      ...
      const scrubbed = scrubResponseOutputForDeletedAgentTasks(
        value,
      ...
      expect(
        JSON.parse(
          typeof scrubbed[1].output === 'string' ? scrubbed[1].output : '{}'
        ).images
      ).toEqual([
  ...
  it('scrubs raw payloads without changing malformed or already clean tasks', () => {
    ...
    const payloadTask = task('payload', {
      rawResponsePayload: JSON.stringify({ id: 'response-a', output }),
    })
    ...
    expect(JSON.parse(scrubbed.rawResponsePayload ?? '{}')).toEqual({
      id: 'response-a',
      output: [output[1]],
    })
    expect(
...
describe('Agent response recovery derivation', () => {
  it('derives single and historical batch outputs without duplicating completed calls', () => {
      ...
      responseOutput: [
        {
          ...
          name: 'generate_image',
          call_id: 'single-call',
          arguments: JSON.stringify({ id: ' single-id ' }),
        },
        {
          type: 'function_call',
          name: 'generate_image_batch',
    ...
    const recovered = createReadyAgentRecoveredToolState(value, [
      single,
      batchA,
      batchB,
    ])
    ...
    expect(
      recovered?.additions.map((item) =>
        JSON.parse(typeof item.output === 'string' ? item.output : '{}')
      )
    ).toEqual([
  ...
  it('waits for pending recoverable tasks and accepts an already paired completed state', () => {
    ...
    expect(
      createReadyAgentRecoveredToolState(pendingRound, [pending])
    ).toBeNull()
  ...
  it('waits when a completed recovery item is paired but another call is recoverable', () => {
    const completed = task('completed', {
      agentRoundId: 'round-a',
      agentToolCallId: 'completed-call',
    })
    const pending = task('pending', {
      agentRoundId: 'round-a',
    ...
    expect(
      createReadyAgentRecoveredToolState(value, [completed, pending])
    ).toBeNull()
  })
  ...
  it('counts recovered calls defensively and derives stable failure messages', () => {
    const output: ResponsesOutputItem[] = [
      ...
      {
        type: 'function_call_output',
        output: '{"images":[{"status":"done"},{"status":"error"}]}',
      },
      { type: 'function_call_output', output: '{bad json' },
      {
        type: 'function_call_output',
        output: [{ type: 'input_text', text: '{"status":"done"}' }],
      },
      { type: 'image_generation_call' },
    ]
    const doneTasks = [
    ...
    const failedA = task('failed-a', {
      status: 'error',
      outputImages: [],
      error: '明确失败',
    })
    const failedB = task('failed-b', {
      status: 'error',
      outputImages: [],
      error: null,
    })
    expect(
      getAgentRecoveredFailureError(round({ outputTaskIds: [failedA.id] }), [
        failedA,
      ])
    ).toBe('明确失败')
    expect(
      getAgentRecoveredFailureError(
        round({ outputTaskIds: [failedA.id, failedB.id] }),
        [failedA, failedB]
      )
    ).toBe('部分图像生成任务失败。')

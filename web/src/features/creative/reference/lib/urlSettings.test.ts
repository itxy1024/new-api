...
async function importPresetConfigOnlyUrlSettings(
  options: { locked?: boolean; multiple?: boolean } = {}
) {
  vi.resetModules()
...
describe('URL settings params', () => {
  ...
  it('creates and activates a new OpenAI profile for legacy URL params', () => {
    ...
    const next = normalizeSettings({
      ...
      ...buildSettingsFromUrlParams(
        current,
        new URLSearchParams('apiUrl=https://api.example.com/v1&apiKey=test-key')
      ),
    })
  ...
  it('uses and updates the profile ID from an OpenAI share URL', () => {
    ...
    const first = normalizeSettings({
      ...
      ...buildSettingsFromUrlParams(
        current,
        new URLSearchParams(
          'profileId=shared-openai&apiUrl=https://api.example.com/v1&model=model-v1'
        )
      ),
    })
    const second = normalizeSettings({
      ...
      ...buildSettingsFromUrlParams(
        first,
        new URLSearchParams(
          'profileId=shared-openai&apiUrl=https://api.example.com/v2&model=model-v2'
        )
      ),
    })

    expect(
      second.profiles.filter((profile) => profile.id === 'shared-openai')
    ).toHaveLength(1)
    expect(
      second.profiles.find((profile) => profile.id === 'shared-openai')
    ).toMatchObject({
  ...
  it('preserves fields omitted from a same-ID OpenAI share URL', () => {
    ...
    const next = normalizeSettings({
      ...
      ...buildSettingsFromUrlParams(
        current,
        new URLSearchParams(
          'profileId=shared-openai&apiUrl=https://new.example.com/v1&profileName=Shared'
        )
      ),
    })
    ...
    expect(
      next.profiles.find((profile) => profile.id === existingProfile.id)
    ).toMatchObject({
      name: 'Shared',
  ...
  it('uses model from URL params for OpenAI profiles', () => {
    ...
    const next = normalizeSettings({
      ...
      ...buildSettingsFromUrlParams(
        current,
        new URLSearchParams(
          'apiUrl=https://api.example.com/v1&apiKey=test-key&model=custom-image-model'
        )
      ),
    })
  ...
  it('uses reasoning effort from URL params for Responses profiles', () => {
    ...
    const next = normalizeSettings({
      ...
      ...buildSettingsFromUrlParams(
        current,
        new URLSearchParams('apiMode=responses&reasoningEffort=max')
      ),
    })
  ...
  it('uses profile name from URL params for OpenAI profiles', () => {
    ...
    const next = normalizeSettings({
      ...
      ...buildSettingsFromUrlParams(
        current,
        new URLSearchParams(
          'apiUrl=https://api.example.com/v1&apiKey=test-key&profileName=测试配置'
        )
      ),
    })
  ...
  it('creates a separate profile when the API URL trailing slash differs', () => {
    ...
    const next = normalizeSettings({
      ...
      ...buildSettingsFromUrlParams(
        current,
        new URLSearchParams(
          'apiUrl=https://api.example.com/v1/&apiKey=test-key'
        )
      ),
    })
  ...
  it('creates a separate profile when URL profile name differs', () => {
    ...
    const next = normalizeSettings({
      ...
      ...buildSettingsFromUrlParams(
        current,
        new URLSearchParams(
          'apiUrl=https://api.example.com/v1/&apiKey=test-key&profileName=URL Profile'
        )
      ),
    })
  ...
  it('creates a separate profile when URL codex CLI option differs', () => {
    ...
    const next = normalizeSettings({
      ...
      ...buildSettingsFromUrlParams(
        current,
        new URLSearchParams(
          'apiUrl=https://api.example.com/v1/&apiKey=test-key&codexCli=true'
        )
      ),
    })
    const activeProfile = next.profiles.find(
  ...
  it('creates a separate profile when URL streaming options differ', () => {
    ...
    const next = normalizeSettings({
      ...
      ...buildSettingsFromUrlParams(
        current,
        new URLSearchParams(
          'apiUrl=https://api.example.com/v1/&apiKey=test-key&streamImages=true&streamPartialImages=3'
        )
      ),
    })
    const activeProfile = next.profiles.find(
  ...
  it('creates an OpenAI profile from legacy params even when fal is active', () => {
    ...
    const next = normalizeSettings({
      ...
      ...buildSettingsFromUrlParams(
        current,
        new URLSearchParams(
          'apiUrl=https://api.example.com/v1&apiKey=openai-key'
        )
      ),
    })
  ...
  it('applies the codex CLI query parameter to a requested custom profile', () => {
    ...
    const next = normalizeSettings({
      ...
      ...buildSettingsFromUrlParams(
        current,
        new URLSearchParams('profileId=custom-profile&codexCli=true')
      ),
    })
  ...
  it('applies the transparent background method to the active fal profile', () => {
    ...
    const next = normalizeSettings({
      ...
      ...buildSettingsFromUrlParams(
        current,
        new URLSearchParams('transparentBackgroundMethod=local')
      ),
    })
  ...
  it('applies the transparent background method to a requested custom profile', () => {
    ...
    const next = normalizeSettings({
      ...
      ...buildSettingsFromUrlParams(
        current,
        new URLSearchParams(
          'profileId=custom-profile&transparentBackgroundMethod=local'
        )
      ),
    })
  ...
  it('clears known URL setting params without touching unrelated params', () => {
    const params = new URLSearchParams(
      'reasoningEffort=high&transparentBackgroundMethod=local&foo=bar'
    )
  ...
  it('imports settings with custom providers from URL params', () => {
    ...
    expect(next.customProviders[0]).toMatchObject({
      id: 'custom-json',
      name: 'Custom JSON',
    })
    expect(next.activeProfileId).toBe('custom-profile')
  ...
  it('patches the active profile instead of creating a new one when only default config is shown', async () => {
    const { buildSettingsFromUrlParams } =
      await importPresetConfigOnlyUrlSettings()
    const current = normalizeSettings(DEFAULT_SETTINGS)
    const next = normalizeSettings({
      ...current,
      ...buildSettingsFromUrlParams(
        current,
        new URLSearchParams(
          'apiUrl=https://api.example.com/v1&apiKey=test-key&model=custom-model&profileName=导入配置&apiMode=responses'
        )
      ),
    })
  ...
  it('patches and activates the preset profile selected by profileId', async () => {
    const { buildSettingsFromUrlParams } =
      await importPresetConfigOnlyUrlSettings({ multiple: true })
    ...
    const params = new URLSearchParams(
      'profileId=preset-b&apiUrl=https://preset-b.example.com/v1'
    )
    params.set(
      'settings',
      JSON.stringify({
    ...
    expect(
      next.profiles.find((profile) => profile.id === profileA.id)
    ).toMatchObject({
      name: 'Preset A',
      apiKey: 'key-a',
      model: 'model-a',
    ...
    expect(
      next.profiles.find((profile) => profile.id === profileB.id)
    ).toMatchObject({
      name: 'Preset B',
  ...
  it('ignores a same-ID settings profile with a conflicting provider in preset-only mode', async () => {
    ...
    expect(
      next.profiles.find((profile) => profile.id === 'preset-b')
    ).toMatchObject({
      provider: 'openai',
      apiKey: 'query-key',
  ...
  it('only applies the API key from URL parameters when preset parameters are locked', async () => {
    const { buildSettingsFromUrlParams } =
      await importPresetConfigOnlyUrlSettings({ locked: true })
    const current = normalizeSettings(DEFAULT_SETTINGS)
    const next = normalizeSettings({
      ...current,
      ...buildSettingsFromUrlParams(
  ...
  it('ignores an invalid explicit profileId in preset-only mode', async () => {
    ...
    expect(
      buildSettingsFromUrlParams(
        current,
        new URLSearchParams('profileId=missing-preset&apiKey=changed-key')
      )
    ).toEqual({})
  })
})

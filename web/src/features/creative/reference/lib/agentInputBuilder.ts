...
function createGeneratedImageReferencePart(
  round: AgentRound,
  task: TaskRecord,
  imageIndex: number
) {
  ...
  const promptAttribute = truncatedPrompt
    ? ` prompt="${escapeXmlAttribute(truncatedPrompt)}"`
    : ''
  return {
    type: 'input_text',
...
async function createBatchImagesInputItem(
  round: AgentRound,
  tasks: TaskRecord[],
  batchTaskIds: string[],
  loadImage: LoadImage
...
export async function buildAgentApiInput(
  ...
  const input: unknown[] = []
  const rounds = getAgentRoundPath(
    options.conversation,
    options.currentRound.id
  )

  for (const round of rounds) {
    const userMessage = options.conversation.messages.find(
      (message) => message.id === round.userMessageId
    )
    if (!userMessage) continue

    input.push(
      await createUserInputItem(
        options.conversation,
        round,
        userMessage,
        options.tasks,
    ...
    if (output?.length) {
      ...
      if (sanitizedOutput.length > 0) {
        ...
        const assistantMessage = round.assistantMessageId
          ? options.conversation.messages.find(
              (message) => message.id === round.assistantMessageId
            )
          : null
        input.push(
          createAssistantFallbackItem(
    ...
    if (round.outputTaskIds.length > 0) {
      const imagesItem = await createGeneratedImagesInputItem(
        round,
        options.tasks,
        options.loadImage
      )
...
export async function buildAgentContinuationInput(
  options: BuildAgentContinuationInputOptions
): Promise<unknown[]> {
  const functionCallOutputIds = new Set(
  ...
  const currentRoundOutput = options.currentRoundOutput.filter(
    (item) =>
      ...
      !item.call_id ||
      !functionCallOutputIds.has(item.call_id)
  )
  const input = [
    ...options.baseInput,
  ...
  const batchImagesItem = await createBatchImagesInputItem(
    options.currentRound,
    options.tasks,
    options.batchTaskIds,
    options.loadImage
  )
  if (batchImagesItem) input.push(batchImagesItem)

  const newImageRefs = collectAgentRoundOutputImageSlots(
    options.currentRound,
    options.tasks
  )
    .map((imageId, index) =>
      imageId
        ? `<ref id="${getAgentGeneratedImageReferenceId(options.currentRound, index)}" />`
        : null
    )

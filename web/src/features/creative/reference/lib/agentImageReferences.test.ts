...
describe('agent image references', () => {
  it('extracts generated and current input XML reference ids', () => {
    expect(
      extractAgentReferenceIds(
        '参考 <ref id="round-1-image-2" /> 和 <ref id="round-3-reference-1" />'
      )
    ).toEqual(['round-1-image-2', 'round-3-reference-1'])
  })
  ...
  it('resolves previous round image references from visible text', () => {
    ...
    expect(
      resolveAgentPromptImageReferences('参考 @第1轮图2 和 @2轮图1', rounds, [
  ...
  it('keeps previous round image numbering stable after a task is removed', () => {
    const rounds = [
      round({ index: 1, outputTaskIds: ['task-deleted', 'task-live'] }),
    ]

    expect(
      resolveAgentPromptImageReferences('参考 @第1轮图1 和 @第1轮图2', rounds, [
  ...
  it('replaces removed previous round references with removed_ref tags', () => {
    ...
    expect(
      ...
      '参考 <removed_ref id="round-1-image-1" /> 和 <ref id="round-1-image-2" /> 生成'
    )
  })
})

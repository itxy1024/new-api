...
describe('imageCache', () => {
  ...
  it('evicts the least recently used images and thumbnails', async () => {
    ...
    await expect(
      ensureImageThumbnailCached('thumbnail-0')
    ).resolves.toMatchObject({ dataUrl: 'thumbnail-data-0' })
    ...
    await expect(
      ensureImageThumbnailCached('thumbnail-1')
    ).resolves.toMatchObject({ dataUrl: 'stored-thumbnail' })
  ...
  it('prioritizes visible thumbnail backfills and notifies subscribers', async () => {
    ...
    await vi.waitFor(() =>
      expect(db.getImageThumbnail).toHaveBeenCalledTimes(2)
    )

    expect(db.getImage.mock.calls.map(([id]) => id)).toEqual([
      'visible',
      'background',
    ])
    expect(db.getImageThumbnail.mock.calls.map(([id]) => id)).toEqual([
      'visible',
      'background',
    ])

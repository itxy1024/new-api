...
function createAgentInstructions(settings: AppSettings, codexCliSize?: string) {
  ...
  if (codexCliSize && codexCliSize !== 'auto') {
    instructions.push(
      '',
      `- Start every image prompt with exactly "Generate at ${codexCliSize} resolution." followed by a space.`
    )
  }

  if (settings.agentMathFormattingPrompt)
    instructions.push('', AGENT_MATH_FORMATTING_INSTRUCTIONS)
    ...
    parameters: {
      ...
      properties: {
        id: {
          type: 'string',
          description:
            'Short stable identifier for this image, e.g. "cover", "base_character", "scene_1".',
        },
        prompt: {
          type: 'string',
          description:
            'Complete image generation prompt with all visual details. Include matching XML ref tags when referring to existing images.',
        },
      },
      required: ['id', 'prompt'],
...
function createAgentTools(
  ...
  const singleImageToolInstruction =
    settings.agentApiConfigMode === 'hybrid'
...
function getErrorMessageFromValue(value: unknown): string | null {
  ...
  return (
    getStringValue(value, 'message') ?? getStringValue(value, 'code') ?? null
  )
}
  ...
  try {
    const body: Record<string, unknown> = {
      ...
      instructions: createAgentInstructions(
        settings,
        (imageProfile ?? profile).codexCli ? params.size : undefined
      ),
      input,
  ...
  try {
    const referenceMapping =
      referenceImageDataUrls.length > 0
        ...
        : ''
    const promptText = allowPromptRewrite
      ? prompt
      : `${PROMPT_REWRITE_GUARD_PREFIX}\n${prompt}`
    const guardedPrompt = [referenceMapping, promptText]
      .filter(Boolean)
      .join('\n\n')
    let input: unknown
    ...
    if (!response.ok) {
      ...
      return {
        batchItemId,
        image: null,
        error: maybeAppendStreamingHint(
          errorMsg,
          response.status,
          profile.streamImages
        ),

...
function eventToImageResponseItem(
  event: Record<string, unknown>
): ImageResponseItem {
  return {
...
async function parseImagesApiStreamResponse(
  ...
  const actualParamsList = completedItems.map((item) =>
    mergeActualParams(pickActualParams(item))
  )
...
function getResponsesStreamPayload(
  event: Record<string, unknown>
...
async function parseResponsesApiStreamResponse(
  ...
  try {
    ...
    const collectedImageItems = outputItems.filter((item) =>
      getResponsesImageResultBase64(item.result)
    )
    if (collectedImageItems.length === 0) throw err
    imageResults = parseResponsesImageResults(
      { output: collectedImageItems },
      mime
    )
  }
...
async function callImagesApiSingle(
  ...
  const sizePrompt =
    profile.codexCli && !opts.skipCodexCliSizePrompt
  ...
  try {
    ...
    if (isEdit) {
      ...
      if (profile.streamImages) {
        ...
        formData.append(
          'partial_images',
          String(getStreamPartialImages(profile))
        )
      }
      ...
      const maskBlob = opts.maskDataUrl
        ? await maskDataUrlToPngBlob(opts.maskDataUrl)
        : null
      if (opts.maskDataUrl) {
    ...
    if (profile.streamImages && isEventStreamResponse(response)) {
      return parseImagesApiStreamResponse(
        response,
        mime,
        opts.onPartialImage,
        controller.signal
...
function createCustomProviderContext(
  opts: CallApiOptions,
  profile: ApiProfile
) {
  const sizePrompt =
...
async function createCustomMultipartBody(
  ...
  const needsInputImages = mapping.files?.some(
    (file) => file.source === 'inputImages'
  )
...
async function extractCustomImages(
  ...
  const imageUrls = (result.imageUrlPaths ?? []).flatMap((path) =>
    getAllByPath(payload, path).filter(
      (value): value is string => isHttpUrl(value) || isDataUrl(value)
    )
  )
  ...
  while (true) {
    if (isFirstPoll) {
      ...
      await new Promise((resolve) =>
        setTimeout(resolve, (poll.intervalSeconds ?? 5) * 1000)
      )
    }
    ...
    if (state === 'failure') {
      ...
      const errorMessage =
        typeof message === 'string' && message.trim() ? message : '异步任务失败'
...
async function callCustomHttpImageApi(
  ...
  try {
    ...
    const useApiProxy = shouldUseApiProxy(profile.apiProxy, proxyConfig)
    const submitMapping =
      isEdit && customProvider.editSubmit
        ? customProvider.editSubmit
        : customProvider.submit
    if (useApiProxy && (submitMapping.method ?? 'POST') !== 'POST') {
      throw new Error(
        'API 代理暂不支持使用 GET 提交的自定义服务商。请关闭 API 代理，或改用 POST 提交的自定义服务商配置。'
      )
    }
    if (useApiProxy && (submitMapping.taskIdPath || customProvider.poll)) {
      throw new Error(
        'API 代理暂不支持使用异步任务的自定义服务商。请关闭 API 代理，或改用同步返回图片的自定义服务商配置。'
      )
    }
    const submitPayload = await submitCustomRequest(
      submitMapping,
      opts,
      profile,
      controller,
    ...
    const taskIdValue = submitMapping.taskIdPath
      ? getByPath(submitPayload, submitMapping.taskIdPath)
      : undefined
    const taskId =
    ...
    if (!taskId)
      return extractCustomImages(
        submitPayload,
        submitMapping.result ?? {},
        mime,
        controller.signal
      )
    if (!customProvider.poll)
      throw new Error('异步接口返回了 task_id，但服务商配置缺少 poll')
    opts.onCustomTaskEnqueued?.({ taskId })
    if (timeoutId) {
      clearTimeout(timeoutId)
      timeoutId = null
    }
    return pollCustomTaskResult(
      profile,
      customProvider.poll,
      taskId,
      mime,
...
function mergeConcurrentApiResults(
  results: PromiseSettledResult<CallApiResult>[]
): CallApiResult {
  ...
  const rawImageUrls = successfulResults.flatMap(
    (result) => result.rawImageUrls ?? []
  )
  ...
  return {
    images,
    actualParams: mergeActualParams(successfulResults[0].actualParams ?? {}, {
      n: images.length,
    }),
    actualParamsList,
    revisedPrompts,
...
async function callResponsesImageApi(
  ...
  const promises = Array.from({ length: n }).map((_, requestIndex) =>
  ...
  try {
    ...
    const body: Record<string, unknown> = {
      ...
      input: createResponsesInput(
        requestPrompt,
        inputImageDataUrls,
        opts.settings.allowPromptRewrite
      ),
      tools: [
        createResponsesImageTool(
          ...
          profile,
          opts.maskDataUrl,
          opts.nativeTransparentBackground
        ),
      ],
    ...
    if (profile.streamImages && isEventStreamResponse(response)) {
      return parseResponsesApiStreamResponse(
        response,
        mime,
        opts.onPartialImage
      )

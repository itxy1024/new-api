...
function getProfileDedupKey(
  profile: Pick<
    AppSettings['profiles'][number],
    | 'provider'
    | 'baseUrl'
    | 'apiKey'
    | 'model'
...
export function getUrlSettingsPayload(
...
function ensureUrlSettingsProfileIds(
  value: unknown,
  ...
  const usedIds = new Set(
    normalizeSettings(currentSettings).profiles.map((profile) => profile.id)
  )
  ...
  return {
    ...
    profiles: record.profiles.map((item) => {
      ...
      if (
        ...
        profile.id.trim() &&
        !emittedIds.has(profile.id.trim())
      ) {
...
export function activateFirstImportedProfile(
  ...
  const importedById = settings.profiles.find(
    (profile) => profile.id === importedProfile.id
  )
  if (importedById)
    return normalizeSettings({ ...settings, activeProfileId: importedById.id })
  const activeProfile = findEquivalentApiProfile(
    settings,
    importedProfile,
 ...
 */
function buildPresetConfigOnlySettingsFromUrlParams(
  currentSettings: Partial<AppSettings> | unknown,
  searchParams: URLSearchParams,
  apiKeyOnly = false
): Partial<AppSettings> {
  ...
  const requestedProfile =
    requestedProfileId && isPresetProfile(requestedProfileId)
  ...
  const importedSettings = ensureUrlSettingsProfileIds(
    getUrlSettingsPayload(searchParams),
    currentSettings
  )
  if (
    importedSettings &&
      ...
      const matched = (
        requestedProfile
          ? profiles.find((item) => {
              ...
              const profile = item as Record<string, unknown>
              return (
                profile.id === requestedProfile.id &&
                (profile.provider === undefined ||
                  profile.provider === targetProfile.provider)
              )
            })
          : profiles.find((item) => {
              if (!item || typeof item !== 'object' || Array.isArray(item))
              ...
              return (
                provider === undefined || provider === targetProfile.provider
              )
            })
      ...
      if (matched) {
        ...
        if (!apiKeyOnly) {
          if (typeof matched.name === 'string' && matched.name.trim())
            patch.name = matched.name.trim()
          if (typeof matched.baseUrl === 'string')
            patch.baseUrl = matched.baseUrl
          if (typeof matched.model === 'string' && matched.model.trim())
            patch.model = matched.model.trim()
          if (
            typeof matched.timeout === 'number' &&
            Number.isFinite(matched.timeout)
          )
            patch.timeout = matched.timeout
          if (typeof matched.apiProxy === 'boolean')
            patch.apiProxy = matched.apiProxy
          if (matched.responseFormatB64Json === true)
            patch.responseFormatB64Json = true
          if (
            matched.transparentBackgroundMethod === 'api' ||
            matched.transparentBackgroundMethod === 'local'
          ) {
            patch.transparentBackgroundMethod =
              matched.transparentBackgroundMethod
          }
          if (
            targetProfile.provider !== 'fal' &&
            typeof matched.codexCli === 'boolean'
          )
            patch.codexCli = matched.codexCli
          if (isOpenAI) {
            if (matched.apiMode === 'images' || matched.apiMode === 'responses')
              patch.apiMode = matched.apiMode
            if (matched.reasoningEffort !== undefined)
              patch.reasoningEffort = normalizeReasoningEffort(
                matched.reasoningEffort
              )
            if (typeof matched.streamImages === 'boolean')
              patch.streamImages = matched.streamImages
            if (matched.streamPartialImages !== undefined)
              patch.streamPartialImages = normalizeStreamPartialImages(
                matched.streamPartialImages
              )
          }
        }
      }
  ...
  const transparentBackgroundMethodParam = searchParams.get(
    'transparentBackgroundMethod'
  )
  ...
  if (!apiKeyOnly) {
    ...
    if (apiUrlParam !== null)
      patch.baseUrl = normalizeBaseUrl(apiUrlParam.trim())
    if (modelParam !== null && modelParam.trim())
      patch.model = modelParam.trim()
    if (
      transparentBackgroundMethodParam === 'api' ||
      transparentBackgroundMethodParam === 'local'
    ) {
  ...
  if (targetProfile.provider !== 'fal' && !apiKeyOnly) {
    ...
    if (codexCliParam !== null)
      patch.codexCli = codexCliParam.trim().toLowerCase() === 'true'
  }
  if (isOpenAI && !apiKeyOnly) {
    ...
    if (apiModeParam === 'images' || apiModeParam === 'responses')
      patch.apiMode = apiModeParam
    if (reasoningEffortParam !== null)
      patch.reasoningEffort = normalizeReasoningEffort(reasoningEffortParam)
    if (streamImagesParam !== null)
      patch.streamImages = streamImagesParam.trim().toLowerCase() === 'true'
    if (streamPartialImagesParam !== null)
      patch.streamPartialImages = normalizeStreamPartialImages(
        streamPartialImagesParam
      )
  }
  ...
  return normalizeSettings({
    ...
    profiles: settings.profiles.map((profile) =>
      profile.id === targetProfile.id
        ? { ...profile, ...patch, provider: profile.provider }
        : profile
    ),
...
function buildRegularSettingsFromUrlParams(
  currentSettings: Partial<AppSettings> | unknown,
  searchParams: URLSearchParams
): Partial<AppSettings> {
  const importedSettings = ensureUrlSettingsProfileIds(
  ...
  const transparentBackgroundMethod:
    | ApiProfile['transparentBackgroundMethod']
    | undefined =
    transparentBackgroundMethodParam === 'api' ||
    transparentBackgroundMethodParam === 'local'
  ...
  const apiMode: ApiMode | undefined =
    apiModeParam === 'images' || apiModeParam === 'responses'
      ? apiModeParam
      : undefined
  ...
  const hasLegacyOpenAIParams =
    ...
    apiMode !== undefined ||
    modelParam !== null ||
    profileNameParam !== null ||
    reasoningEffortParam !== null ||
    streamImagesParam !== null ||
    streamPartialImagesParam !== null
  const settings =
  ...
  const requestedNonOpenAIProfile = requestedProfileId
    ? settings.profiles.find(
        (item) => item.id === requestedProfileId && item.provider !== 'openai'
      )
    : undefined
  if (
    requestedNonOpenAIProfile &&
    (transparentBackgroundMethod !== undefined ||
      (requestedNonOpenAIProfile.provider !== 'fal' && codexCliParam !== null))
  ) {
    return normalizeSettings({
      ...settings,
      profiles: settings.profiles.map((item) =>
        item.id === requestedProfileId
          ? {
              ...item,
              ...(requestedNonOpenAIProfile.provider !== 'fal' &&
              codexCliParam !== null
                ? { codexCli: codexCliParam.trim().toLowerCase() === 'true' }
                : {}),
              ...(transparentBackgroundMethod
                ? { transparentBackgroundMethod }
                : {}),
  ...
  if (
    !requestedProfileId &&
    transparentBackgroundMethod !== undefined &&
    !hasLegacyOpenAIParams
  ) {
    return normalizeSettings({
      ...settings,
      profiles: settings.profiles.map((item) =>
        item.id === settings.activeProfileId
          ? { ...item, transparentBackgroundMethod }
          : item
      ),
    })
  }
  ...
  if (hasLegacyOpenAIParams || transparentBackgroundMethod !== undefined) {
    ...
    if (existingById) {
      ...
      if (transparentBackgroundMethod !== undefined)
        patch.transparentBackgroundMethod = transparentBackgroundMethod

      return normalizeSettings({
        ...settings,
        profiles: settings.profiles.map((item) =>
          item.id === requestedProfileId ? { ...item, ...patch } : item
        ),
        activeProfileId: requestedProfileId,
      })
    }
    ...
    const profile = createDefaultOpenAIProfile({
      id:
        requestedProfileId ||
        createUrlProfileId(new Set(settings.profiles.map((item) => item.id))),
      name: 'URL 参数配置',
      apiMode: profileApiMode,
      model:
        profileApiMode === 'responses'
          ? DEFAULT_RESPONSES_MODEL
          : DEFAULT_IMAGES_MODEL,
    })
    if (apiUrlParam !== null)
      profile.baseUrl = normalizeBaseUrl(apiUrlParam.trim())
    if (apiKeyParam !== null) profile.apiKey = apiKeyParam.trim()
    if (modelParam !== null && modelParam.trim())
      profile.model = modelParam.trim()
    if (reasoningEffortParam !== null)
      profile.reasoningEffort = normalizeReasoningEffort(reasoningEffortParam)
    if (profileName) profile.name = profileName
    if (codexCliParam !== null)
      profile.codexCli = codexCliParam.trim().toLowerCase() === 'true'
    if (streamImagesParam !== null)
      profile.streamImages = streamImagesParam.trim().toLowerCase() === 'true'
    if (streamPartialImagesParam !== null)
      profile.streamPartialImages = normalizeStreamPartialImages(
        streamPartialImagesParam
      )
    if (transparentBackgroundMethod !== undefined)
      profile.transparentBackgroundMethod = transparentBackgroundMethod

    const conflictingById = requestedProfileId
      ? settings.profiles.find((item) => item.id === requestedProfileId)
      : null
    if (conflictingById) {
      return normalizeSettings({
        ...settings,
        profiles: settings.profiles.map((item) =>
          item.id === requestedProfileId
            ? { ...profile, isDefault: item.isDefault }
            : item
        ),
        activeProfileId: requestedProfileId,
      })
...
export function buildSettingsFromUrlParams(
  ...
  const result = isPresetConfigOnlyEnabled()
    ? buildPresetConfigOnlySettingsFromUrlParams(
        currentSettings,
        searchParams,
        isPresetConfigParamsLocked()
      )

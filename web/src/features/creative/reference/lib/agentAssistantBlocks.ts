...
function taskMatchesImageOutputItem(
  task: TaskRecord,
...
export function getAgentAssistantBlocks(
  ...
  for (const [outputIndex, item] of outputItems.entries()) {
    ...
    const imageCallKey =
      item.type === 'image_generation_call' && item.id
    ...
    const matchingIndexes = uniqueTaskSlots
      .map((slot, index) =>
        slot.task && taskMatchesImageOutputItem(slot.task, item) ? index : -1
      )
      .filter((index) => index >= 0)
    const expectedTaskCount = singleImageCall
      ? 1
      : Math.max(getBatchImageCount(item), matchingIndexes.length)
    const taskCount =
      expectedTaskCount > uniqueTaskSlots.length && matchingIndexes.length > 0
    ...
    let start = -1
    for (
      let candidate = nextSlotIndex;
      candidate + taskCount <= uniqueTaskSlots.length;
      candidate += 1
    ) {
      const candidateSlots = uniqueTaskSlots.slice(
        candidate,
        candidate + taskCount
      )
      if (
        candidateSlots.some(
          (slot) => slot.task && !taskMatchesImageOutputItem(slot.task, item)
        )
      )
        continue
      if (
        matchingIndexes.some(
          (index) => index < candidate || index >= candidate + taskCount
        )
      )
        continue
      start = candidate
  ...
  const projectedTaskIds = new Set(
    imageCallProjections.flatMap((projection) =>
      projection.slots.map((slot) => slot.taskId)
    )
  )
  ...
  for (const [slotIndex, slot] of uniqueTaskSlots.entries()) {
    ...
    const next = imageCallProjections.find(
      (projection) => projection.start > slotIndex
    )
    if (next) {
  ...
  const imageCallProjectionByOutputIndex = new Map(
    imageCallProjections.map((projection) => [
  ...
  const flushWebSearchGroup = () => {
    ...
    if (status)
      blocks.push({
        ...
        key: `web-search:${webSearchGroupStart}`,
      })
    webSearchGroup = []
  ...
  for (const [outputIndex, item] of outputItems.entries()) {
    ...
    if (item.type === 'message') {
      ...
      if (content) {
        ...
        blocks.push({
          type: 'text',
          key: `text:${item.id ?? outputIndex}:${outputIndex}`,
          content,
        })
      }
    }
...
export function getAgentAssistantCopyContent(
...
export function getRoundTaskSlots(
  round: AgentRound | null,
  tasks: TaskRecord[]
): AgentRoundTaskSlot[] {
  if (!round) return []

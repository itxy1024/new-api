...
describe('task lifecycle patches', () => {
  ...
  it('marks legacy and OpenAI running tasks as interrupted', () => {
    ...
    const customSyncRunning = task({
      id: 'custom-sync-running',
      apiProvider: 'custom-provider',
      status: 'running',
      createdAt: 5_000,
      finishedAt: null,
    ...
    expect(
      result.tasks.find((item) => item.id === 'custom-sync-running')
    ).toMatchObject({ status: 'error', error: '请求中断' })
...
describe('task actual params', () => {
  it('fills missing sizes without overriding API-returned sizes', () => {
    expect(
      addImageSizeParam({ output_format: 'png' }, { width: 1254, height: 1254 })
    ).toEqual({
      output_format: 'png',
      size: '1254x1254',
    })
    expect(
      addImageSizeParam({ size: '1024x1024' }, { width: 1254, height: 1254 })
    ).toEqual({ size: '1024x1024' })
    expect(
      addImageSizeParam(undefined, { width: 0, height: 1254 })
    ).toBeUndefined()
  })

  it('maps only non-empty actual params to matching output ids', () => {
    const paramsList: Array<Partial<TaskParams> | undefined> = [
      undefined,
      {},
      { size: '1024x1024' },
      { output_format: 'webp' },
    ...
    expect(
      mapActualParamsByImage(['image-a', 'image-b', 'image-c'], paramsList)
    ).toEqual({
  ...
  it('keeps fal and async custom params per image while deriving gallery totals for other providers', () => {
    const paramsList: Array<Partial<TaskParams>> = [
      { output_format: 'png', size: '1254x1254' },
    ]

    expect(
      deriveGalleryActualParams('fal', false, { n: 8 }, paramsList, 1)
    ).toEqual(paramsList[0])
    expect(
      deriveGalleryActualParams(
        'custom-provider',
        true,

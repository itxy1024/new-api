...
export function normalizeCodexCliImageSize(size: string) {
  ...
  if (normalized.width * normalized.height > MAX_1K_PIXELS) {
    return (
      calculateImageSize('1K', `${normalized.width}:${normalized.height}`) ??
...
export function stripInjectedCodexCliSizePrompt(
  prompt: string,
...
export function formatImageRatio(width: number, height: number) {
  ...
  const friendlyNearest = Array.from(
    { length: 12 },
    (_, widthIndex) => widthIndex + 1
  )
    .flatMap((friendlyWidth) =>
      Array.from({ length: 12 }, (_, heightIndex) => heightIndex + 1).map(
        (friendlyHeight) => {
  ...
  return friendlyNearest && friendlyNearest.delta <= 0.04
    ? `≈${friendlyNearest.label}`
    : simplified
}

...
export default function Lightbox() {
  ...
  const isInputImage = Boolean(
    lightboxImageId && inputImages.some((image) => image.id === lightboxImageId)
  )
  ...
  const handleReplaceFileUpload = useCallback(
    async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
        ...
        const targetIdx = currentImages.findIndex(
        ...
        if (
          currentImages.some(
            (item, idx) => idx !== targetIdx && item.id === image.id
          )
        ) {
        ...
        const nextList = lightboxImageList.map((id) =>
          id === targetId ? image.id : id
        )
    ...
    <>
      <LightboxInner
        ...
        editDisabled={Boolean(
          maskDraft && maskDraft.targetImageId !== lightboxImageId
        )}
  ...
  return (
    ...
    >
      ...
      <div className='animate-zoom-in relative'>
        ...
        >
          <img
            ...
            className='saveable-image max-h-[70vh] max-w-[90vw] rounded-lg object-contain shadow-2xl sm:max-h-[75vh] sm:max-w-[85vw]'
      ...
      {showInputActions && !isZoomed && (
        <div
          className='absolute bottom-8 left-1/2 z-10 flex w-max -translate-x-1/2 items-center gap-2 rounded-2xl border border-gray-200/80 bg-white/90 p-2 shadow-2xl backdrop-blur-xl transition-colors dark:border-white/15 dark:bg-black/60'
          onClick={(e) => e.stopPropagation()}
        >
          <button
            type='button'
            className='flex items-center justify-center gap-2 rounded-xl px-5 py-2.5 text-sm font-medium whitespace-nowrap text-gray-700 transition hover:bg-gray-100 active:scale-95 dark:text-white/90 dark:hover:bg-white/15'
            onClick={onReplace}
          >
          ...
          >
            <ButtonTooltip
              visible={editDisabled && editHint.visible}
              text='只能有一张遮罩图'
            />
            <button
              type='button'
              disabled={editDisabled}
              className={`flex items-center justify-center gap-2 rounded-xl px-5 py-2.5 text-sm font-medium whitespace-nowrap shadow-md transition active:scale-95 ${editDisabled ? 'cursor-not-allowed bg-gray-100 text-gray-400 shadow-none dark:bg-white/10 dark:text-white/40' : 'bg-blue-500 text-white hover:bg-blue-600 hover:shadow-blue-500/25'}`}
              onClick={onEdit}
            >
      ...
      {showZoomBadge && isZoomed && zoomPercent !== 100 && (
        <div className='pointer-events-none absolute bottom-6 left-1/2 -translate-x-1/2'>
          <span className='rounded-full border border-gray-200/80 bg-white/90 px-3 py-1.5 text-xs text-gray-800 backdrop-blur-sm transition-opacity duration-500 dark:border-transparent dark:bg-black/50 dark:text-white/80'>
            {zoomPercent}%
          </span>
        </div>
      )}
      {showNav && !isZoomed && (
        <div className='pointer-events-none absolute top-6 left-1/2 z-10 -translate-x-1/2'>
          <span className='rounded-full border border-gray-200/80 bg-white/90 px-3 py-1 text-sm font-medium text-gray-800 shadow-lg backdrop-blur-md dark:border-white/15 dark:bg-black/60 dark:text-white/90'>

...
export default function SearchBar() {
  ...
  const setActiveFavoriteCollectionId = useStore(
    (s) => s.setActiveFavoriteCollectionId
  )
  const openManageCollectionsModal = useStore(
    (s) => s.openManageCollectionsModal
  )
  const failedCount = useStore((s) => {
    ...
    return s.tasks.filter((task) => {
      ...
      if (s.filterFavorite) {
        ...
        if (
          s.activeFavoriteCollectionId &&
          s.activeFavoriteCollectionId !== ALL_FAVORITES_COLLECTION_ID &&
          !getTaskFavoriteCollectionIds(
            task,
            s.defaultFavoriteCollectionId
          ).includes(s.activeFavoriteCollectionId)
        )
          return false
      }
  ...
  useEffect(() => {
    const handleDocumentMouseDown = (event: MouseEvent) => {
      ...
      if (
        target.closest('.task-card-wrapper, .favorite-collection-card-wrapper')
      )
        return
      ...
      .filter((task) => {
        ...
        if (state.filterFavorite) {
          ...
          if (
            ...
            state.activeFavoriteCollectionId !== ALL_FAVORITES_COLLECTION_ID &&
            !getTaskFavoriteCollectionIds(
              task,
              state.defaultFavoriteCollectionId
            ).includes(state.activeFavoriteCollectionId)
          )
            return false
        }
        return taskMatchesSearchQuery(task, q)
    ...
    <div ref={rootRef} data-no-drag-select className='mt-6 mb-4 flex gap-3'>
      <div className='z-20 flex flex-shrink-0 gap-2'>
        ...
        >
          {activeFavoriteCollectionId ? (
            ...
            <FavoriteIcon filled={filterFavorite} className='h-5 w-5' />
          )}
        </SearchActionButton>
        {inCollectionOverview && (
          <SearchActionButton
            ...
            className='rounded-xl border border-gray-200 bg-white p-2.5 text-gray-400 transition-all hover:bg-gray-50 dark:border-white/[0.08] dark:bg-gray-900 dark:hover:bg-white/[0.06]'
          >
      ...
      <div className='relative z-10 flex-1'>
        ...
        <input
          ...
          type='text'
          placeholder={
            inCollectionOverview ? '搜索收藏夹名称...' : '搜索提示词、参数...'
          }

...
export default function ViewportTooltip({
  ...
  useEffect(() => {
    ...
    const updatePosition = () => {
      ...
      const left = Math.min(
        Math.max(anchorCenter - tooltipRect.width / 2, margin),
        maxLeft
      )
    ...
    <>
      ...
      {effectiveVisible &&
        createPortal(
          <div
            ref={tooltipRef}
            className={`pointer-events-none fixed rounded-lg bg-gray-800 px-3 py-2 text-xs font-normal text-white shadow-lg ${className}`}
            style={{

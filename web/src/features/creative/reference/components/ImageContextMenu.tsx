...
export default function ImageContextMenu() {
  ...
  useEffect(() => {
    ...
    const onContextMenu = (e: MouseEvent) => {
      ...
      if (target && target.tagName === 'IMG') {
        ...
        if (!canCopyImage && imgTarget.classList.contains('object-contain'))
          return
        ...
        setMenuInfo({
          ...
          outputImageIds:
            imgTarget.dataset.outputImageIds?.split(',').filter(Boolean) ?? [],
          canCopyImage,
          x: e.clientX,
          y: e.clientY,
        })
  ...
  useEffect(() => {
    ...
    const close = (e: Event) => {
      ...
      if (
        ...
        e.target.closest('[data-lightbox-root]')
      ) {
        window.dispatchEvent(
          new Event('image-context-menu-dismiss-lightbox-click')
        )
      }
    ...
    try {
      ...
      if (outputImageIds[0]) {
        ...
        const matchedTask = tasks.find((t) =>
          t.outputImages?.includes(outputImageIds[0])
        )
        if (matchedTask) {
          fileNameBase = `task-${matchedTask.id}`
        }
      ...
      const result = settings.zipDownloadRoutes.includes(
        'image-context-menu-all'
      )
        ? await downloadImageEntriesAsZip(
            getImageZipEntries(outputImageIds, fileNameBase),
            fileNameBase
          )
  ...
  const handleEdit = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      console.error(err)
      showToast(
        `加入参考图失败：${err instanceof Error ? err.message : String(err)}`,
        'error'
      )
    }
  }
  ...
  const menuItemCount =
    (menuInfo.canCopyImage ? 1 : 0) + 1 + (showDownloadAll ? 1 : 0) + 1
  ...
  return (
    <div
      ref={menuRef}
      className='animate-fade-in fixed z-[9999] w-[120px] overflow-hidden rounded-lg border border-gray-100 bg-white py-1 shadow-xl dark:border-gray-700 dark:bg-gray-800'
      style={{ left, top }}

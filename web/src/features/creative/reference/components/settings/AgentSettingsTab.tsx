  ...
  return (
    <div className='space-y-4'>
      <div className='block'>
        <div className='mb-1 flex items-center justify-between gap-3'>
          ...
          <div className='w-20 shrink-0'>
            <Select
              ...
              onChange={(value) =>
                updateAgentApiConfigMode(value as AgentApiConfigMode)
              }
              options={[
                { label: '关闭', value: 'off' },
        ...
        >
          <div>
            原生：使用原生的 Responses API 配置，由模型调用{' '}
            <code className='rounded bg-gray-100 px-1 py-0.5 font-mono text-[10px] dark:bg-white/[0.06]'>
              image_generation
            </code>{' '}
            工具生成图片。
          </div>
          <div>
            混合：使用非原生的混合 API
            配置，由文本模型调用自定义工具，请求图像模型生成图像，解决部分服务商/模型不支持{' '}
            <code className='rounded bg-gray-100 px-1 py-0.5 font-mono text-[10px] dark:bg-white/[0.06]'>
              image_generation
            </code>{' '}
            工具的问题。
                ...
                {agentTextProfileOptions.length > 0 ? (
                  <Select
                    ...
                    onChange={(value) =>
                      commitSettings({
                        ...draft,
                        agentTextProfileId: String(value),
                      })
                    }
                  ...
                  {agentImageProfileOptions.length > 0 ? (
                    <Select
                      ...
                      onChange={(value) =>
                        commitSettings({
                          ...draft,
                          agentImageProfileId: String(value),
                        })
                      }
      ...
      <label className='block'>
        ...
        <div
          data-selectable-text
          className='mt-1.5 text-xs leading-relaxed text-gray-500 dark:text-gray-500'
        >
      ...
      <div className='block'>
        <div className='mb-1 flex items-center justify-between gap-3'>
          ...
          <button
            ...
            onClick={() => {
              ...
              commitSettings({
                ...draft,
                agentMaxToolRounds,
                agentWebSearch: !draft.agentWebSearch,
              })
            }}
          ...
          >
            <span
              className={`inline-block h-3 w-3 transform rounded-full bg-white shadow transition-transform ${draft.agentWebSearch ? 'translate-x-[14px]' : 'translate-x-[2px]'}`}
            />
          </button>
        </div>
        <div

  ...
  return (
    <div className='space-y-4'>
      <div className='hidden sm:block'>
        <div className='mb-1 flex items-center justify-between'>
          ...
          <div className='w-28 shrink-0'>
            <Select
              ...
              onChange={(val) =>
                commitSettings({ ...draft, enterSubmit: val === 'enter' })
              }
              options={[
                {
        ...
        >
          选择{' '}
          {navigator.userAgent.includes('Mac') ? '⌘ + Enter' : 'Ctrl + Enter'}{' '}
        ...
        <div className='mb-1 flex items-center justify-between'>
          ...
          <button
            ...
            onClick={() =>
              commitSettings({
                ...draft,
                allowPromptRewrite: !draft.allowPromptRewrite,
              })
            }
          ...
          >
            <span
              className={`inline-block h-3 w-3 transform rounded-full bg-white shadow transition-transform ${draft.allowPromptRewrite ? 'translate-x-[14px]' : 'translate-x-[2px]'}`}
            />
          </button>
        </div>
        <div
          ...
          className='text-xs text-gray-500 dark:text-gray-500'
        >
          开启后，Codex CLI 兼容模式下的 Image API 请求和所有 Responses API
          请求都不再附加防改写提示词，允许模型按服务商策略优化提示词。
        </div>
      </div>
      <div className='block'>
        <div className='mb-1 flex items-center justify-between'>
      ...
      <div className='block'>
        <div className='mb-1 flex items-center justify-between'>
          ...
          <button
            ...
            onClick={() =>
              commitSettings({
                ...draft,
                agentMathFormattingPrompt: !draft.agentMathFormattingPrompt,
              })
            }
          ...
          >
            <span
              className={`inline-block h-3 w-3 transform rounded-full bg-white shadow transition-transform ${draft.agentMathFormattingPrompt ? 'translate-x-[14px]' : 'translate-x-[2px]'}`}
            />
          </button>
        </div>
        <div
        ...
        >
          开启后，Agent 会被要求使用{' '}
          <code className='rounded bg-gray-100 px-1 py-0.5 font-mono text-[0.9em] text-gray-700 dark:bg-white/10 dark:text-gray-200'>
            $...$
          </code>{' '}
          和{' '}
          <code className='rounded bg-gray-100 px-1 py-0.5 font-mono text-[0.9em] text-gray-700 dark:bg-white/10 dark:text-gray-200'>

...
function drawMaskImageToCanvas(
...
export default function MaskEditorModal() {
  ...
  function commitViewTransform(nextTransform: ViewTransform) {
    ...
    const clamped = frame
      ? clampViewTransform(nextTransform, {
  ...
  function updateCursor(point: Point | null) {
    ...
    const radius =
      (brushSize / 2 / maskCanvas.width) * frame.clientWidth * scale
  ...
  function getViewportCenterCanvasPoint(): Point | null {
    ...
    return {
      x:
        ((frame.clientWidth / 2 - transform.x) /
          transform.scale /
          frame.clientWidth) *
        maskCanvas.width,
      y:
        ((frame.clientHeight / 2 - transform.y) /
          transform.scale /
          frame.clientHeight) *
        maskCanvas.height,
    }
  ...
  function drawAt(point: Point, nextTool = tool) {
    ...
    ctx.globalCompositeOperation =
      nextTool === 'brush' ? 'destination-out' : 'source-over'
    ctx.fillStyle = '#fff'
    ctx.beginPath()
  ...
  useEffect(() => {
    ...
    return () =>
      document.removeEventListener('pointerdown', closeBrushControls, true)
  }, [showBrushControls])
  ...
  const isZoomed =
    ...
    Math.abs(viewTransform.x) > 1 ||
    Math.abs(viewTransform.y) > 1
  ...
  return (
    <>
      <div
        data-no-drag-select
        className='animate-modal-in fixed inset-0 z-[80] flex flex-col bg-gray-50 dark:bg-gray-900'
      >
        {/* Header */}
        <div className='z-20 flex flex-none items-center justify-between border-b border-gray-200 bg-white px-4 py-3 sm:px-6 sm:py-3.5 dark:border-gray-800 dark:bg-gray-950'>
          <div className='flex items-center gap-3'>
            <button
              onClick={close}
              disabled={isSaving}
              className='-ml-2 rounded-lg p-2 text-gray-500 transition hover:bg-gray-100 sm:rounded-xl sm:p-2.5 dark:text-gray-400 dark:hover:bg-gray-800'
              title='取消'
            >
              <svg
            ...
            <div className='relative flex items-center gap-1.5 sm:gap-2'>
              <h2
                className='text-sm font-semibold text-gray-800 sm:text-base dark:text-gray-200'
              ...
              <button
                ...
                className='flex h-6 w-6 items-center justify-center rounded-full text-gray-400 transition hover:bg-gray-100 hover:text-gray-600 sm:h-7 sm:w-7 dark:text-gray-500 dark:hover:bg-gray-800 dark:hover:text-gray-300'
                aria-label='遮罩编辑说明'
              >
                <svg
              ...
              {showMaskInfo && (
                <div className='absolute top-full left-0 mt-2 w-64 rounded-xl border border-gray-200/80 bg-white px-3.5 py-2.5 text-xs leading-5 text-gray-600 shadow-lg sm:w-72 sm:text-sm sm:leading-6 dark:border-white/[0.08] dark:bg-gray-900 dark:text-gray-300'>
                  <div className='absolute -top-1.5 left-16 h-3 w-3 rotate-45 border-t border-l border-gray-200/80 bg-white dark:border-white/[0.08] dark:bg-gray-900' />
                  <p>
                    根据官方文档说明，此功能仅基于提示词，无法完全控制模型编辑区域。
                  </p>
                  <p className='mt-2'>
                    建议附加类似“只编辑遮罩区域”的提示词以提升模型指令遵循程度。
                  </p>
                </div>
              )}
            </div>
          ...
          <div className='flex items-center gap-2 sm:gap-3'>
            {maskDraft?.targetImageId === imageId && (
              <button
                ...
                className='flex h-8 items-center gap-1.5 rounded-xl bg-gray-100 px-3.5 text-xs font-medium text-gray-700 transition hover:bg-red-50 hover:text-red-600 sm:h-[38px] sm:px-4 sm:text-sm dark:bg-white/[0.08] dark:text-gray-300 dark:hover:bg-red-500/20 dark:hover:text-red-400'
              >
                移除遮罩
              </button>
            )}
            <button
              ...
              className='flex h-8 items-center gap-1.5 rounded-xl bg-blue-500 px-4 text-xs font-medium text-white shadow-sm transition hover:bg-blue-600 active:bg-blue-700 disabled:opacity-50 sm:h-[38px] sm:px-5 sm:text-sm'
            >
              {isSaving ? '保存中...' : '保存'}
            </button>
          </div>
        ...
        <div
          ref={stageRef}
          className='relative flex flex-1 items-center justify-center overflow-hidden bg-gray-100/50 p-0 pb-[76px] sm:p-6 sm:pb-[100px] dark:bg-black/50'
        ...
        >
          ...
          <div
            ref={baseFrameRef}
            className='relative max-h-full max-w-full touch-none shadow-inner ring-black/5 sm:rounded-xl sm:ring-1 dark:bg-black/50 dark:ring-white/5'
            onWheel={handleWheel}
            style={{
              aspectRatio: size ? `${size.width} / ${size.height}` : '1 / 1',
              width: size
                ? `min(100%, 100cqh * ${size.width / size.height})`
                : '520px',
              maxHeight: '100%',
            }}
          >
        ...
        {/* Footer Toolbar */}
        <div className='pointer-events-none absolute bottom-4 left-1/2 z-20 flex w-full -translate-x-1/2 items-center justify-center px-2 sm:bottom-8 sm:px-4'>
          <div className='pointer-events-auto flex items-center gap-2 rounded-2xl border border-gray-200/80 bg-white/95 px-2 py-1.5 shadow-2xl backdrop-blur-md sm:gap-4 sm:rounded-[1.25rem] sm:px-3 sm:py-2 dark:border-white/5 dark:bg-[#0f0f0f]/95'>
            <div className='flex items-center gap-1.5 sm:gap-3'>
              <div className='flex items-center rounded-xl bg-gray-100/80 p-1 sm:rounded-[14px] dark:bg-[#232325]/80'>
                <button
                  className={`rounded-lg p-2 transition-all sm:rounded-xl sm:p-2.5 ${tool === 'brush' ? 'bg-white text-blue-500 shadow-sm dark:bg-[#323338] dark:text-blue-400 dark:shadow-none' : 'text-gray-500 hover:text-gray-700 dark:text-[#8a8a8e] dark:hover:text-gray-200'}`}
                ...
                <button
                  className={`rounded-lg p-2 transition-all sm:rounded-xl sm:p-2.5 ${tool === 'eraser' ? 'bg-white text-blue-500 shadow-sm dark:bg-[#323338] dark:text-blue-400 dark:shadow-none' : 'text-gray-500 hover:text-gray-700 dark:text-[#8a8a8e] dark:hover:text-gray-200'}`}
                ...
                >
                  ...
                  >
                    <g transform='translate(0, 1) rotate(-45 12 12)'>
                      <path
                        fill='currentColor'
                        d='M4 10a2 2 0 0 1 2-2h7v8H6a2 2 0 0 1-2-2z'
                      />
              ...
              <div
                ref={brushSizeControlRef}
                className='relative flex items-center justify-center'
              >
                <button
                  ref={brushSizeButtonRef}
                  onClick={toggleBrushControls}
                  className={`flex h-10 w-10 items-center justify-center rounded-xl border transition-all sm:h-[46px] sm:w-[46px] sm:rounded-[14px] ${showBrushControls ? 'border-blue-200 bg-blue-50 text-blue-600 dark:border-[#323338] dark:bg-[#323338] dark:text-blue-400' : 'border-gray-200/80 bg-white text-gray-700 hover:bg-gray-50 dark:border-[#323338] dark:bg-transparent dark:text-[#e0e0e0] dark:hover:border-[#3a3b40] dark:hover:bg-white/[0.04]'}`}
                  disabled={!isReady || isSaving}
                  title='调节笔刷大小'
                >
            ...
            <div className='flex items-center gap-0.5 sm:ml-1 sm:gap-2'>
              <button
                onClick={handleUndo}
                disabled={!canUndo}
                className='rounded-lg p-2 text-gray-500 transition-all hover:bg-gray-100 disabled:opacity-30 sm:rounded-xl sm:p-2.5 dark:text-[#8a8a8e] dark:hover:bg-white/10 dark:hover:text-gray-200'
                title='撤销'
              >
                <svg
                  className='h-5 w-5'
              ...
              <div className='mx-1 h-4 w-px bg-gray-300 sm:h-5 dark:bg-[#323338]'></div>
              <button
      ...
      {showBrushControls &&
        ...
        createPortal(
          <div
            ...
            className='pointer-events-auto fixed z-[100] flex h-44 w-12 -translate-x-1/2 touch-none items-center justify-center rounded-full border border-gray-200/80 bg-white/95 shadow-[0_8px_30px_rgb(0,0,0,0.12)] backdrop-blur-md select-none dark:border-white/[0.08] dark:bg-[#28292d]/95'
            style={{
              left: sliderAnchor.left,
              bottom: sliderAnchor.bottom + 8,
              touchAction: 'none',
            }}
            onPointerDown={(e) => {
          ...
          >
            <input
              ...
              className='pointer-events-none h-1.5 w-32 -rotate-90 cursor-ns-resize appearance-none rounded-full bg-gray-200 outline-none dark:bg-black/30 [&::-webkit-slider-thumb]:h-4 [&::-webkit-slider-thumb]:w-4 [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-blue-500 [&::-webkit-slider-thumb]:shadow-md'
              disabled={!isReady || isSaving}
            />
          </div>,
          document.body

...
} from './lib/inputDraftState'
import { orderInputImagesForMask } from './lib/mask'
import { getNewApiSelection, type NewApiSelection } from './lib/newApiSelection'
...
import {
  ...
  mergePersistedAgentConversations,
  migratePersistedState,
  normalizePersistedState,
} from './lib/persistedState'
import {
  enforcePresetConfigPolicy,
  getPresetConfig,
  getPresetProfileIds,
  getPresetProviderIds,
  isPresetConfigDeletionPrevented,
  isPresetConfigOnlyEnabled,
  isPresetConfigParamsLocked,
  isPresetProfile,
...
const deletedActiveAgentTasks = new Map<
  string,
  { task: TaskRecord; controller: AbortController }
>()
...
async function replaceStoredAgentConversations(
...
function mergePersistedState(
  ...
  if (plan.shouldMigrateAgentConversations)
...
export const useStore = create<AppState>()(
  persist(
    (set, get) => ({
      ...
      dismissPresetProfile: (id) =>
        set((state) => ({
          dismissedPresetProfileIds: state.dismissedPresetProfileIds.includes(
            id
          )
            ? state.dismissedPresetProfileIds
            : [...state.dismissedPresetProfileIds, id],
        })),
      restorePresetProfile: (id) =>
        set((state) => ({
          dismissedPresetProfileIds: state.dismissedPresetProfileIds.filter(
            (item) => item !== id
          ),
        })),
      dismissedPresetProviderIds: [],
      dismissPresetProvider: (id) =>
        set((state) => ({
          dismissedPresetProviderIds: state.dismissedPresetProviderIds.includes(
            id
          )
            ? state.dismissedPresetProviderIds
            : [...state.dismissedPresetProviderIds, id],
        })),
      restorePresetProvider: (id) =>
        set((state) => ({
          dismissedPresetProviderIds: state.dismissedPresetProviderIds.filter(
            (item) => item !== id
          ),
        })),
      setSettings: (s) =>
        set((st) => {
          ...
          const effectiveDismissedPresetProviderIds =
            st.dismissedPresetProviderIds.filter(
              (id) => !isPresetProviderDeletionPrevented(id, previous.profiles)
            )
          const settings = normalizeSettings(
            enforcePresetConfigPolicy(merged, {
      ...
      setPresetImportedSettings: async (importedSettings, transform) => {
        set((state) => {
          ...
          const dismissedPresetProfileIds =
            state.dismissedPresetProfileIds.filter((id) => presetIds.has(id))
          const deletionPrevented = isPresetConfigDeletionPrevented()
          const remainingPresetProfiles = (
            getPresetConfig()?.profiles ?? state.settings.profiles
          ).filter((profile) => !dismissedPresetProfileIds.includes(profile.id))
          const dismissedPresetProviderIds =
            state.dismissedPresetProviderIds.filter((id) =>
              presetProviderIds.has(id)
            )
          const effectiveDismissedPresetProviderIds =
            dismissedPresetProviderIds.filter(
              (id) =>
                !isPresetProviderDeletionPrevented(id, remainingPresetProfiles)
            )
          const merged = mergePresetImportedSettings(
            state.settings,
            importedSettings,
            {
              lockPresetParams: isPresetConfigParamsLocked(),
              dismissedPresetProfileIds: deletionPrevented
                ? []
                : dismissedPresetProfileIds,
              dismissedPresetProviderIds: effectiveDismissedPresetProviderIds,
              previousPresetConfig: state.previousPresetConfig,
              usedPresetProfileIds: state.tasks.flatMap((task) =>
                task.apiProfileId ? [task.apiProfileId] : []
              ),
            }
          )
          const settings = normalizeSettings(
            enforcePresetConfigPolicy(
              normalizeSettings(
                transform?.(merged.settings) ?? merged.settings
              ),
              {
                dismissedPresetProviderIds: effectiveDismissedPresetProviderIds,
              }
          ...
          return {
            settings,
            previousPresetConfig: getPresetConfig()
              ? merged.presetConfig
              : null,
            dismissedPresetProfileIds,
            dismissedPresetProviderIds,
            reusedTaskApiProfileId: shouldClearReusedProfile
              ? null
              : state.reusedTaskApiProfileId,
            ...(shouldClearReusedProfile
              ? {
      ...
      replaceInputImage: (idx, img) => {
        ...
        set((s) => {
          ...
          return syncActiveInputDraft(
            s,
            updateInputDraftImages(s, inputImages, {
      ...
      moveInputImage: (fromIdx, toIdx) =>
        set((s) => {
          ...
          return syncActiveInputDraft(
            s,
            updateInputDraftImages(s, images, { clearMissingMask: false })
          )
        }),
      maskDraft: null,
      ...
      deleteAgentRound: (conversationId, roundId) =>
        deleteAgentRoundAndTasks(conversationId, roundId),
      deleteAgentAssistantMessage: (conversationId, messageId) =>
        deleteAgentAssistantMessageAndTasks(conversationId, messageId),
...
function usesConcurrentImageRequests(
  settings: AppSettings,
  profile: ApiProfile,
  params: TaskParams,
  hasInputImages: boolean
  ...
  if (profile.provider === 'openai') {
    ...
    return (
      profile.apiMode === 'images' && (profile.codexCli || profile.streamImages)
    )
  }
  return (
    profile.provider !== 'fal' &&
    profile.codexCli &&
    !isAsyncCustomProviderTask(settings, profile.provider, hasInputImages)
  )
}
...
function getFalRecoveryProfile(settings: AppSettings, task: TaskRecord) {
  ...
  if (taskProfile?.provider === 'fal' && task.apiProvider === 'fal')
    return taskProfile
  return null
}
...
function getCustomRecoveryProfile(settings: AppSettings, task: TaskRecord) {
  ...
  if (
    taskProfile &&
    taskProfile.provider === task.apiProvider &&
    taskProfile.provider !== 'openai' &&
    taskProfile.provider !== 'fal'
  )
    return taskProfile
  return null
}
...
/** 将 NewAPI 选择器中的 Key/模型转换为可执行的临时 API 配置。 */
function applyNewApiSelection(
  settings: AppSettings,
  selection: NewApiSelection
): { settings: AppSettings; profile: ApiProfile } {
  const normalized = normalizeSettings(settings)
  const existing = normalized.profiles.find(
  ...
  const profiles = existing
    ? normalized.profiles.map((item) => (item.id === 'newapi' ? profile : item))
  ...
  return {
    ...
    profile:
      nextSettings.profiles.find((item) => item.id === 'newapi') ?? profile,
  }
}

function getAgentProfileValidationError(
  settings: AppSettings
): { profile: ApiProfile | null; message: string } | null {
  const normalized = normalizeSettings(settings)
  const textProfile = getAgentTextApiProfile(normalized)
  if (
    !textProfile ||
    textProfile.provider !== 'openai' ||
  ...
  if (textProfileError)
    return {
      profile: textProfile,
      message: `文本模型 API 配置不完整：${textProfileError}`,
    }
  ...
  if (normalized.agentApiConfigMode === 'hybrid') {
    ...
    if (imageProfileError)
      return {
        ...
        message: `图像模型 API 配置不完整：${imageProfileError}`,
      }
  }
...
async function resolveImageSizeParamsList(
  ...
  const withStoredSizes = images.map((_, index) =>
    addImageSizeParam(preferred?.[index], sizes?.[index])
  )
  ...
  return images.map((_, index) => {
    ...
    if (fallbackParams?.size)
      return { ...(params ?? {}), size: fallbackParams.size }
...
async function completeRecoveredFalTask(
  ...
  if (
    !latest ||
    latest.status === 'done' ||
    latest.error === AGENT_STOPPED_MESSAGE
  )
    return
  ...
  if (
    !latestBeforeUpdate ||
    latestBeforeUpdate.status === 'done' ||
    latestBeforeUpdate.error === AGENT_STOPPED_MESSAGE ||
    (latestBeforeUpdate.status !== 'running' &&
      !latestBeforeUpdate.falRecoverable)
  ) {
...
export async function initStore() {
  ...
  loadedAgentConversations = mergePersistedAgentConversations(
  ...
  const { tasks: markedTasks, interruptedTasks } =
    markInterruptedOpenAIRunningTasks(storedTasks, Date.now())
  ...
  if (galleryInputDraft) {
    ...
    const shouldClearMask =
      galleryInputDraft.maskDraft !== restoredGalleryDraft.maskDraft
...
export async function submitTask(
  ...
  // 选择器状态可能先于 Zustand 设置同步完成，提交时再次以选择器为准，
  // 确保本次请求不会落到默认配置或默认分组。
  const selectedNewApi = getNewApiSelection()
  if (selectedNewApi) {
    const applied = applyNewApiSelection(normalizedSettings, selectedNewApi)
    normalizedSettings = applied.settings
    activeProfile = applied.profile
    requestSettings = createSettingsForApiProfile(
      normalizedSettings,
      activeProfile
    )
    useStore.getState().setSettings(normalizedSettings)
  }

  // NewAPI 只能使用当前选择器快照，禁止无快照时回退到默认配置。
  if (
    (activeProfile.id === 'newapi' ||
      activeProfile.baseUrl.trim() === '/api/creative') &&
    !selectedNewApi
  ) {
  ...
  const shouldUseTransparentOutput =
    (normalizedParams.output_format === 'png' ||
      normalizedParams.output_format === 'webp') &&
  ...
  const transparentMeta =
    taskParams.transparent_output &&
    activeProfile.transparentBackgroundMethod === 'local'
  ...
  const task: TaskRecord = {
    ...
    ...(selectedNewApi && activeProfile.id === 'newapi'
      ? { newApiKeyId: selectedNewApi.keyId, newApiGroup: selectedNewApi.group }
      : {}),
...
function getDeletedActiveAgentTasks(
  conversationId: string,
  roundId: string,
  ...
  return Array.from(deletedActiveAgentTasks.values())
    .filter(
      (entry) =>
        ...
        entry.task.agentConversationId === conversationId &&
        entry.task.agentRoundId === roundId
    )
  ...
  try {
    for (const dataUrl of images) {
      ...
      if (task.transparentOutput) {
        ...
        try {
          outputDataUrl =
            task.params.output_format === 'webp'
              ? await removeKeyedBackgroundFromDataUrl(
                  dataUrl,
                  undefined,
                  'webp',
...
function scrubAgentOutputPayloadsForDeletedTasks(deletedTasks: TaskRecord[]) {
  ...
  for (const task of deletedTasks) {
    if (
      ...
      continue
    const key = getAgentRoundControllerKey(
      task.agentConversationId,
      task.agentRoundId
    )
  ...
  useStore.setState((state) => {
    ...
    const agentConversations = state.agentConversations.map((conversation) => {
      ...
      const rounds = conversation.rounds.map((round) => {
        ...
        const responseOutput = scrubResponseOutputForDeletedAgentTasks(
          round,
          round.responseOutput,
          roundDeletedTasks,
          tasksByRound.get(key) ?? roundDeletedTasks
        )
    ...
    const tasks = state.tasks.map((task) => {
      ...
      const scrubbed = scrubTaskRawResponsePayloadForDeletedTasks(
        task,
        round,
        roundDeletedTasks,
        tasksByRound.get(key) ?? roundDeletedTasks
...
async function persistTaskDeletionCleanup(
  ...
  const conversations = cleanup.updatedConversations.map(
  ...
  try {
    ...
    await Promise.all([
      ...
      ...conversations.map((conversation) =>
        dbPutAgentConversation(conversation)
      ),
    ])
  }
}

function appendAgentRecoveredToolOutputs(
  conversationId: string,
  roundId: string,
  additions: ResponsesOutputItem[]
) {
  updateAgentConversation(conversationId, (current) => ({
    ...current,
    updatedAt: Date.now(),
    rounds: current.rounds.map((round) => {
      ...
      const nextAdditions = additions.filter(
        (item) => item.call_id && !existingOutputCallIds.has(item.call_id)
      )
...
async function continueRecoveredAgentRound(taskId: string) {
  ...
  if (agentRoundControllers.has(key) || agentRecoveryContinuations.has(key))
    return
  ...
  try {
    ...
    const conversation = latestState.agentConversations.find(
      (item) => item.id === task.agentConversationId
    )
    const round = conversation?.rounds.find(
      (item) => item.id === task.agentRoundId
    )
    if (
      !conversation ||
      !round ||
      round.status === 'done' ||
      round.error === AGENT_STOPPED_MESSAGE
    )
      return
    ...
    const recovered = createReadyAgentRecoveredToolState(
      round,
      latestState.tasks
    )
    if (!recovered) return
    ...
    const updatedConversation = updatedState.agentConversations.find(
      (item) => item.id === conversation.id
    )
    const updatedRound = updatedConversation?.rounds.find(
    ...
    const agentValidationError =
      getAgentProfileValidationError(normalizedSettings)
    if (agentValidationError) {
    ...
    const roundTasks = updatedState.tasks.filter(
      (item) => item.agentRoundId === round.id
    )
    const resumeParams =
      roundTasks.find((item) => item.params)?.params ??
      normalizeParamsForSettings(
        updatedState.params,
        createSettingsForApiProfile(normalizedSettings, imageProfile),
        { hasInputImages: round.inputImageIds.length > 0 }
      )
    const toolCallsUsed = getAgentRecoveredToolCallCount(
      updatedRound.responseOutput ?? [],
      roundTasks
    )
...
export async function submitAgentMessage() {
  ...
  if (agentValidationError) {
    showToast(
      `请先完善 Agent API 配置：${agentValidationError.message}`,
      'error'
    )
    state.setShowSettings(
      true,
      normalizedSettings.agentApiConfigMode === 'off' ? 'api' : 'agent'
    )
    return
  }
...
async function executeAgentRound(
  ...
  try {
    ...
    const assistantMessageId = existingAssistantMessage?.id ?? genId()
    const resumedAssistantContent = resume
    ...
    const imageParams = normalizeParamsForSettings(
      params,
    ...
    const getDeletedAgentTasks = () => {
      const deletedTasks = getDeletedActiveAgentTasks(
        conversationId,
        roundId,
        controller
      )
      ...
      const currentTaskIds = new Set(
        useStore.getState().tasks.map((task) => task.id)
      )
      for (const [toolCallId, taskId] of taskIdByToolCallId) {
        const task = taskByToolCallId.get(toolCallId)
        if (task && !currentTaskIds.has(taskId) && !deletedTaskIds.has(taskId))
    ...
    const ensureStreamingAgentTask = async (
      ...
      const task: TaskRecord = {
        ...
        ...(options.agentBatchCallId
          ...
          : {}),
        ...(options.agentBatchItemId
          ? { agentBatchItemId: options.agentBatchItemId }
          : {}),
      }
    ...
    const completeAgentImageTask = async (
      ...
      const actualParams = deriveAgentImageActualParams(
        image.actualParams,
        stored
      )
      updateTaskInStore(taskId, {
    ...
    const pauseAgentImageTaskForRecovery = (
      toolCallId: string,
      err: unknown
    ) => {
      const taskId = taskIdByToolCallId.get(toolCallId)
      ...
      if (
        ...
        latestTask.falRequestId &&
        latestTask.falEndpoint
      ) {
        useStore.getState().setTaskStreamPreview(taskId)
        updateTaskInStore(taskId, {
          ...createTaskErrorPatch(
            latestTask,
            '与 fal.ai 的连接已断开，之后会继续查询任务结果。',
            Date.now()
          ),
          falRecoverable: true,
        })
    ...
    let accumulatedOutputItems: ResponsesOutputItem[] =
      resume?.responseOutput ?? []
    let accumulatedText = resumedAssistantContent
    const textSegments: string[] = resumedAssistantContent
      ? [resumedAssistantContent]
      : []
    ...
    const parseSingleImageCallArguments = (
      args: string
    ...
    const callHybridImageApiSingle = async (opts: {
      ...
      const result = await callImageApi({
        ...
        onPartialImage: opts.onPartialImage
          ? (partial) => {
              void opts.onPartialImage?.({
                image: partial.image,
                partialImageIndex:
                  partial.partialImageIndex ?? partial.requestIndex,
              })
            }
          : undefined,
    ...
    const executeSingleImageFunctionCall = async (
      ...
      const item = parseSingleImageCallArguments(
      ...
      const taskParams = {
        ...normalizeParamsForSettings(imageParams, imageRequestSettings, {
      ...
      try {
        ...
        if (controller.signal.aborted) throw createAgentAbortError()
        if (result.image) {
          const completed = await completeAgentImageTask(
        ...
        if (controller.signal.aborted) throw createAgentAbortError()
        if (pauseAgentImageTaskForRecovery(toolCallId, err))
          throw createAgentRecoveryPauseError()
    ...
    const executeBatchFunctionCall = async (
      ...
      const batchPromises = batchExecutionItems.map(
        async ({
          ...
          const latestTask = useStore
            .getState()
            .tasks.find(
              (task) => task.id === taskIdByToolCallId.get(batchToolCallId)
            )
          return {
            ...batchResult,
            committed:
              ...
              Boolean(
                ...
                latestTask.outputImages.length > 0
              ),
          }
        }
      )
      ...
      for (let i = 0; i < batchItems.length; i++) {
        ...
        const taskId = taskIdByToolCallId.get(
          batchExecutionItems[i].batchToolCallId
        )
        if (
          !taskId ||
          !useStore.getState().tasks.some((task) => task.id === taskId)
        )
          continue
        if (settled.status === 'fulfilled') {
          ...
          if (
            isAgentRecoveryPauseError(settled.reason) ||
    ...
    while (true) {
        ...
        onOutputItems: shouldStreamAssistantMessage
          ? (outputItems) => {
              ...
              currentResponseOutputItems =
                canonicalizeBatchFunctionCallArguments(outputItems)
              updateAgentConversation(conversationId, (current) => ({
                ...current,
                rounds: current.rounds.map((item) =>
                  item.id === roundId
      ...
      currentResponseOutputItems = canonicalizeBatchFunctionCallArguments(
        currentResponseOutputItems.length
          ? currentResponseOutputItems
          : (result.outputItems ?? [])
      )
      ...
      const cleanedConversation = useStore
        .getState()
        .agentConversations.find((item) => item.id === conversationId)
      const latestRoundForCleanup = cleanedConversation?.rounds.find(
        (item) => item.id === roundId
      )
      const outputBeforeFunctionResults =
        latestRoundForCleanup?.responseOutput ?? accumulatedOutputItems
      const mergedOutputItems = mergeResponseOutputItems(
        outputBeforeFunctionResults,
        functionCallOutputs
      )
      const accumulatedOutputItemsWithFunctionOutputs = latestRoundForCleanup
        ...
        : mergedOutputItems
      const generatedOutputCallIds = new Set(
        functionCallOutputs.map((item) => item.call_id).filter(Boolean)
      )
      const effectiveFunctionCallOutputs =
        accumulatedOutputItemsWithFunctionOutputs.filter(
          (item) =>
            item.type === 'function_call_output' &&
            item.call_id &&
            generatedOutputCallIds.has(item.call_id)
        )
      ...
      const latestConversation = continuationState.agentConversations.find(
        (item) => item.id === conversationId
      )
    ...
    const taskIds = streamingTaskIds.filter((taskId) =>
      existingTaskIds.has(taskId)
    )
    const outputIds = taskIds.flatMap(
      (taskId) =>
        latestTasks.find((task) => task.id === taskId)?.outputImages ?? []
    )
    ...
    markAgentRoundTasksFailed(
      ...
      roundId,
      message,
      getRawErrorPayload(err).rawResponsePayload
    )
    ...
    try {
      ...
      if (
        cleanup.updatedTasks.length > 0 ||
        cleanup.updatedConversations.length > 0
      ) {
      ...
      for (const task of deletedTasks) {
        if (deletedActiveAgentTasks.get(task.id)?.controller === controller)
          deletedActiveAgentTasks.delete(task.id)
      }
    }
  }
}
...
async function executeTask(taskId: string) {
  ...
  if (!task) return
  let taskProfile = getTaskApiProfile(settings, task)
  let executionSettings = normalizeSettings(settings)
  const taskNewApiSelection =
    task.newApiKeyId && task.apiModel
      ? {
          keyId: task.newApiKeyId,
          model: task.apiModel,
          group: task.newApiGroup ?? '',
        }
      : null
  if (task.apiProfileId === 'newapi' && !taskNewApiSelection) {
    updateTaskInStore(taskId, {
      ...createTaskErrorPatch(
        task,
        '任务缺少 NewAPI Key/模型快照，请重新提交。',
        Date.now()
      ),
      falRecoverable: false,
      customRecoverable: false,
  ...
  if (!taskProfile && taskNewApiSelection) {
    const applied = applyNewApiSelection(executionSettings, taskNewApiSelection)
    executionSettings = applied.settings
    taskProfile = applied.profile
  }
  ...
  const activeProfile = taskProfile ?? getActiveApiProfile(executionSettings)
  const requestSettings = createSettingsForApiProfile(
    executionSettings,
    activeProfile
  )
  const taskProvider =
    taskProfile?.provider ?? task.apiProvider ?? activeProfile.provider
  ...
  try {
    ...
    const result = await callImageApi({
      ...
      nativeTransparentBackground:
        task.params.transparent_output && !task.transparentOutput,
    ...
    const actualParams = deriveGalleryActualParams(
      taskProvider,
      isAsyncCustomTask,
      result.actualParams,
      actualParamsList,
    ...
    const revisedPrompts =
      activeProfile.codexCli && task.sourceMode !== 'agent'
        ? result.revisedPrompts?.map((prompt) =>
            prompt == null
              ? prompt
    ...
    const revisedPromptByImage = shouldStoreRevisedPrompts
      ? mapRevisedPromptsByImage(outputIds, revisedPrompts)
      : undefined
    const promptWasRevised =
      shouldStoreRevisedPrompts &&
      revisedPrompts?.some(
        (revisedPrompt) =>
          revisedPrompt?.trim() && revisedPrompt.trim() !== requestPrompt.trim()
      )
    const hasRevisedPromptValue =
      shouldStoreRevisedPrompts &&
      revisedPrompts?.some((revisedPrompt) => revisedPrompt?.trim())
    if (
      taskProvider === 'openai' &&
      activeProfile.apiMode === 'responses' &&
...
export async function updateTasksFavoriteCollections(
  ...
  const updated = tasks.map((task) => {
    ...
    if (
      sameFavoriteCollectionIds(
        getTaskFavoriteCollectionIds(task, defaultFavoriteCollectionId),
        ids
      )
    )
      return task
  ...
  if (result.updatedTasks.length) {
    ...
    const updated = useStore.getState().tasks.map((task) => {
      ...
      return patch
        ? {
            ...task,
            favoriteCollectionIds: patch.favoriteCollectionIds,
    ...
    await Promise.all(
      updated
        .filter((task) => patches.has(task.id))
        .map((task) => putTask(task))
    )
  }
  if (result.taskIdsToDelete.length)
    await removeMultipleTasks(result.taskIdsToDelete)
...
export async function retryTask(task: TaskRecord) {
  ...
  const appliedSettings = selectedNewApi
    ? applyNewApiSelection(normalizeSettings(settings), selectedNewApi)
    : null
  const effectiveSettings =
    appliedSettings?.settings ?? normalizeSettings(settings)
  const activeProfile =
    appliedSettings?.profile ?? getActiveApiProfile(effectiveSettings)
...
export async function reuseConfig(task: TaskRecord) {
  ...
  const taskProfile = normalizedSettings.reuseTaskApiProfileTemporarily
    ? getTaskApiProfile(normalizedSettings, task)
    : null
  const matchedProfile =
    taskProfile &&
    (!isPresetConfigOnlyEnabled() || isPresetProfile(taskProfile.id))
      ? taskProfile
      : null
...
function getAgentRoundDeletionTaskIds(
  conversation: AgentConversation,
  round: AgentRound,
  tasks: TaskRecord[]
) {
  const messageIds = new Set(
    conversation.messages
      .filter((message) => message.roundId === round.id)
    ...
    ...tasks
      .filter(
        (task) =>
          ...
          Boolean(task.agentMessageId && messageIds.has(task.agentMessageId))
      )
      .map((task) => task.id),
  ])
}

function hasRunningAgentDeletionWork(
  conversationId: string,
  roundIds: Set<string>,
  taskIds: Set<string>,
  state: AppState
  ...
  if (
    conversation?.rounds.some(
      (round) => roundIds.has(round.id) && round.status === 'running'
    )
  )
    return true
  if (
    state.tasks.some(
      (task) => taskIds.has(task.id) && task.status === 'running'
    )
  )
    return true
  for (const roundId of roundIds) {
    ...
    if (
      agentRecoveryContinuations.has(key) &&
      round?.status !== 'done' &&
      round?.error !== AGENT_STOPPED_MESSAGE
    )
      return true
...
function cleanDeletedAgentReferences(
  conversation: AgentConversation,
  taskIds: Set<string>,
  ...
  let changed = false
  const rounds = conversation.rounds.map((round) => {
    const outputTaskIds = round.outputTaskIds.filter(
      (taskId) => !taskIds.has(taskId)
    )
    const clearAssistantMessage = Boolean(
      round.assistantMessageId &&
      assistantMessageIds.has(round.assistantMessageId)
    )
    if (
  ...
  const messages = conversation.messages.map((message) => {
    if (!message.outputTaskIds?.some((taskId) => taskIds.has(taskId)))
      return message
    changed = true
    return {
      ...message,
      outputTaskIds: message.outputTaskIds.filter(
        (taskId) => !taskIds.has(taskId)
      ),
    }
  })
  return changed
    ? { ...conversation, rounds, messages, updatedAt: now }
    : conversation
}

async function deleteAgentRoundAndTasks(
  conversationId: string,
  roundId: string
): Promise<AgentDeletionResult> {
  const state = useStore.getState()
  ...
  const taskIds = new Set(
    getAgentRoundDeletionTaskIds(conversation, round, state.tasks)
  )
  if (
    hasRunningAgentDeletionWork(
      conversationId,
      new Set([roundId]),
      taskIds,
      state
  ...
  try {
    await removeTasks([...taskIds], (latest, deletedTaskIds) => {
      const latestConversation = latest.agentConversations.find(
        (item) => item.id === conversationId
      )
      const latestRound = latestConversation?.rounds.find(
        (item) => item.id === roundId
      ...
      const removedMessageIds = new Set([
        ...
        ...(latestRound.assistantMessageId
          ? [latestRound.assistantMessageId]
          : []),
      ...
      const removedAssistantMessageIds = new Set(
        latestConversation.messages
          .filter(
            (message) =>
              message.role === 'assistant' && removedMessageIds.has(message.id)
          )
          .map((message) => message.id)
      )
      if (latestRound.assistantMessageId)
        removedAssistantMessageIds.add(latestRound.assistantMessageId)
      const now = Date.now()
      const agentConversations = latest.agentConversations.map((item) => {
        const candidate =
          item.id === conversationId
            ? {
                ...nextConversation,
                messages: nextConversation.messages.filter(
                  (message) => !removedMessageIds.has(message.id)
                ),
              }
            : item
        return cleanDeletedAgentReferences(
          candidate,
          deletedTaskIds,
      ...
      const agentInputDrafts = remapAgentInputDraftMentionsForPathChange(
      ...
      return {
        agentConversations,
        agentInputDrafts,
        ...(latest.activeAgentConversationId === conversationId &&
        latest.appMode === 'agent'
          ? {
          ...
          : {}),
        agentEditingRoundId:
          latest.agentEditingRoundId === roundId
            ? null
            : latest.agentEditingRoundId,
      }
    })
  } catch (err) {
...
async function deleteAgentAssistantMessageAndTasks(
  conversationId: string,
  messageId: string
): Promise<AgentDeletionResult> {
  const state = useStore.getState()
  ...
  const taskIds = new Set(
    uniqueIds([
      ...
      ...(round
        ...
        : []),
      ...state.tasks
        .filter((task) => task.agentMessageId === messageId)
        .map((task) => task.id),
    ])
  )
  const referencedRoundIds = new Set([
    message.roundId,
    ...conversation.rounds
      .filter((item) => item.assistantMessageId === messageId)
      .map((item) => item.id),
  ])
  if (
    hasRunningAgentDeletionWork(
...
async function removeTasks(
  ...
  for (const task of deletedTasks) {
    ...
    const controller =
      task.agentConversationId && task.agentRoundId
        ? agentRoundControllers.get(
  ...
  if (updateState) {
    useStore.setState((state) => {
      ...
      if (patch.agentConversations) {
        ...
        domainUpdatedConversations.push(
          ...patch.agentConversations.filter(
            (conversation) => previousById.get(conversation.id) !== conversation
          )
        )
      }
      return patch
  ...
  const updatedConversations = new Map(
    cleanup.updatedConversations.map((conversation) => [
      conversation.id,
      conversation,
    ])
  )
  for (const conversation of domainUpdatedConversations)
    updatedConversations.set(conversation.id, conversation)
  await persistTaskDeletionCleanup(
    deletedTasks.map((task) => task.id),
    {
      ...cleanup,
...
export async function clearFailedTasks(taskIds?: string[]) {
  ...
  if (partialFailedTaskIds.size) {
    ...
    setTasks(updated)
    const nextSelectedTaskIds = selectedTaskIds.filter(
      (id) => !partialFailedTaskIds.has(id)
    )
...
async function completeRecoveredCustomTask(
  ...
  if (
    ...
    latestBeforeUpdate.error === AGENT_STOPPED_MESSAGE ||
    (latestBeforeUpdate.status !== 'running' &&
      !latestBeforeUpdate.customRecoverable)
  ) {
    await deleteUnreferencedImageIds([
      ...outputIds,
      ...(transparentOriginalImageIds ?? []),
  ...
  useStore
    .getState()
    .showToast(`自定义异步任务已恢复，共 ${outputIds.length} 张图片`, 'success')
  if (!isAgentTask(task))
    showTaskCompletionNotification(
      '图像生成完成',
...
export async function exportData(
  ...
  try {
    ...
    if (
      options.exportTasks &&
      hasActiveDataOperations(state.tasks, state.agentConversations)
    )
    ...
    const params = {
      ...
      favoriteCollections,
      defaultFavoriteCollectionId,
      agentConversations: options.exportTasks
    ...
    for (const id of imageIds) {
      ...
      imageSizes.push({
        id,
        bytes: getExportImageEstimatedBytes(image, thumbnail),
      })
    }
    ...
    for (let index = 0; index < plan.length; index++) {
      ...
      const result = await buildExportZip({
        ...
        backupPart:
          plan.length > 1
            ? { id: backupId, index: partNumber, total: plan.length }
            : undefined,
      })
    ...
    useStore
      ...
      .showToast(
        plan.length > 1
          ? `已请求下载 ${plan.length} 个 ZIP，请确认浏览器已允许多文件下载`
          : '数据已导出',
        'success'
      )
  } catch (e) {
    ...
    useStore
      .getState()
      .showToast(detail ? `导出失败，${detail}` : '导出失败，未知错误', 'error')
  }
}
...
export async function restoreExplicitPresetConfig(ids: {
  providerIds: string[]
  profileIds: string[]
}) {
  const presetProviderIds = getPresetProviderIds()
...
export async function importData(
  ...
  try {
    ...
    const selected = [] as Array<{
      file: File
      manifest: Awaited<ReturnType<typeof readExportZipManifest>>
    }>
    for (const file of files) {
      const manifest = await readExportZipManifest(
    ...
    if (multipart) {
      ...
      if (
        options.importTasks &&
        (selected.length !== first.total || indexes.size !== first.total)
      ) {
        throw new Error(
          `分片备份不完整，请一次选择同一备份的全部 ${first.total} 个 ZIP。`
        )
      }
      selected.sort(
        (a, b) => a.manifest.backupPart!.index - b.manifest.backupPart!.index
      )
    }
    ...
    if (
      ...
      !options.importTasks &&
      !settingsManifests.length
    )
      throw new Error('所选备份不包含配置数据。')
    const importedTasks = selected.flatMap((part) => part.manifest.tasks ?? [])
    const importedAgentConversations = selected.flatMap(
      (part) => part.manifest.agentConversations ?? []
    )
    const hasTaskData = selected.some(
      (part) => part.manifest.tasks != null || part.manifest.imageFiles != null
    )

    const importedImageIds: string[] = []
    if (options.importTasks && hasTaskData) {
      for (const part of selected) {
        const { manifest, files: zipFiles } = await readExportZip(
          new Uint8Array(await part.file.arrayBuffer())
        )
        ...
        for (const [id, info] of Object.entries(
          manifest.thumbnailFiles ?? {}
        )) {
          const thumbnailDataUrl = readExportZipFileAsDataUrl(
            zipFiles,
            info.path
          )
      ...
      const state = useStore.getState()
      const importedFavoriteCollections = selected.flatMap(
        (part) => part.manifest.favoriteCollections ?? []
      )
      const mergedFavorites = mergeFavoriteCollections(
        state.favoriteCollections,
        importedFavoriteCollections
      )
      ...
      const importedDefaultFavoriteCollectionId = selected
        ...
        .find(
          (id) =>
            id != null &&
            favoriteCollections.some((collection) => collection.id === id)
        )
      const defaultFavoriteCollectionId = mergedFavorites.importedCollections
        .length
        ? resolveDefaultFavoriteCollectionId(
            favoriteCollections,
      ...
      const normalizedAgentConversations = normalizeAgentConversations(
      ...
      useStore.setState((state) => {
        ...
        const activeAgentConversationId =
          ...
          agentConversations.some(
            ...
            ? state.activeAgentConversationId
            : (normalizedAgentConversations[0]?.id ??
    ...
    if (options.importConfig && settingsManifests.length) {
      ...
      const providerIds = new Set(
        settingsManifests.flatMap(
          (part) =>
            part.manifest.settings?.customProviders.map(
      ...
      const settings = settingsManifests.reduce(
        (current, part) =>
          mergeImportedSettings(current, part.manifest.settings, {
    ...
    useStore
      .getState()
      .showToast(detail ? `导入失败，${detail}` : '导入失败，未知错误', 'error')
    return false
  }
}

...
function LegacyCreativePage({ mode }: { mode: Mode }) {
  ...
  function downloadLatest() {
    ...
    link.download = `${latest.kind}-${latest.id}`
    link.click()
  }
          ...
          <div className='flex h-[4.25rem] items-center justify-between px-5 md:px-10'>
            ...
            <div className='flex items-center gap-2'>
              <div className='bg-muted/60 flex items-center rounded-2xl border p-1'>
                ...
                <button
                  type='button'
                  onClick={() => toast.info(t('Agent'))}
                  className='text-muted-foreground hover:bg-background hover:text-foreground rounded-xl px-5 py-2 text-sm transition'
                >
                  {t('Agent')}
                </button>
              </div>
              ...
              <Button
                variant='ghost'
                size='icon-sm'
                onClick={() => toast.info(t('Advanced options'))}
                aria-label={t('Help')}
              >
